...
class InputSpace(object):
    ...
    def __init__(self, variables_config, batch_size = 1):
        """
	...
	self.consider_single_point = False #Used by Parallel BO in the recommendation method.
	...
	for batch_sample in range(self.batch_size):
		if self.batch_size > 1:
			...
			params = params_backup[batch_sample]
	...
	params = [OrderedDict() for x in range(self.batch_size)]
	
	batch_size = self.batch_size
	if self.consider_single_point:
		self.batch_size = 1
	for suggestion_index in range(self.batch_size):
        	for name, vdict in self.variables_meta.iteritems():
		    indices = vdict['indices']
		    if self.batch_size == 1:
			index = indices
		    else:
		    	index = [indices[0]+self.num_dims*suggestion_index]
	            if vdict['type'] == 'int' or vdict['type'] == 'float':
	                params[suggestion_index][name] = {}
	                params[suggestion_index][name]['type'] = vdict['type']
	                params[suggestion_index][name]['values'] = data_vector[index]
	            elif vdict['type'] == 'enum':
			...
			#    i+=1
			if name not in params[suggestion_index]:
				params[suggestion_index][name] = {}
				params[suggestion_index][name]['type'] = vdict['type']
				params[suggestion_index][name]['values'] = np.array([])
			#intervals =  np.linspace(0,1,len(vdict['options']))
			#index_value = np.argmin(np.absolute(data_vector[indices[0]] - intervals))
			params[suggestion_index][name]['values'] = np.append(params[suggestion_index][name]['values'],data_vector[index])
			if len(params[suggestion_index][name]['values']) == len(vdict['options']): #Enum is filled with all options, can now compute the label
				params[suggestion_index][name]['label'] = vdict['options'][np.argmax(params[suggestion_index][name]['values'])]
	...
	if self.consider_single_point:
		self.consider_single_point = False
		self.batch_size = batch_size
	...
	if self.batch_size > 1:
		return self.vectorify_parallel(params)
    ...
    def vectorify_parallel(self, params):

	multiple = False
	#We assume that all the points of the batch have the same params structure.
	for name, param in params[0].iteritems():
                if len(param['values']) > 1 and (param['type'] == 'int' or param['type'] == 'float'):
                        multiple = True

        if multiple == True:
                return [self.vectorify_multiple(params[i]) for i in range(len(params))]
        else:
                return [self.vectorify_single(params[i]) for i in range(len(params))]
...
def paramify_no_types(input_params):
	...
	for ip in input_params:
		for name, param in ip.iteritems():
                	vals = param['values']
