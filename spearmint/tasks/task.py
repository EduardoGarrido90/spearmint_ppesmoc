...
class Task(object):
    ...
    def valid_normalized_values(self, input_space):
        if self.type == 'objective':
            ...
            # If it's a constraint we only standardize the variance
...
#            dd['values'] = self.standardize_variance(self.valid_values)
	...
	# DHL : added to avoid problems with contrains (the data is not centered) and the first element y[0] can be negative
