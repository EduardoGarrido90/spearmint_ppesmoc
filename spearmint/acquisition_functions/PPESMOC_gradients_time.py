...
def update_full_Factors_only_test_factors(a, damping, minimize=True, no_negative_variances_nor_nands = False, no_negatives = True):
    ...
    g00 = np.dstack([g00, np.zeros((n_test, n_pset, q))])
    g00 = g00.reshape((n_test, n_pset, q, 2))
    g00 = np.dstack([g00, np.zeros((n_test, n_pset, q, 2))])
    ...
    g00 = g00.flatten()
    g00_id = np.array([range(g00.shape[0])])[0]
    insert_indexes_g00 = g00_id[g00_id%2==1][:-1]
    np.insert(g00, insert_indexes_g00, np.array([0.0, 0.0, 0.0]))
    """
...
def compute_acq_fun_wrt_test_points_time(X_test, obj_model_dict, con_models_dict, pareto_set, info_gps, tasks):#, log, fun_log):
            ...
            for ps in pareto_set.keys():
                ...
                print('update marginals 2: ' + str(end-start) + ' seconds.')
                """
