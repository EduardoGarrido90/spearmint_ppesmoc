	...
	def acquisition(self, obj_model_dict, con_models_dict, cand, current_best, compute_grad, minimize=True, tasks=None, tasks_values=None):
		...
		if not key in self.cached_acquisition_on_grid:
		    acq_dict = evaluate_acquisition_function_given_EP_solution(obj_model_dict, cand, epSolution, pareto_set, \
                    ...
                    if cand.shape[ 0 ] == 1 or cand.shape[ 0 ] != self.cached_acquisition_on_grid[ key ][ self.cached_acquisition_on_grid[ key ].keys()[ 0 ] ].shape[ 0 ]:
        		    acq_dict = evaluate_acquisition_function_given_EP_solution(obj_model_dict, cand, epSolution, pareto_set, \
                            minimize=minimize, opt = self.options, con_models=con_models_dict)
                    else:
...
def test_plot_results_decoupled(num_results):
	...
	np.savetxt("R/plot_acq/decoupled/txts/acq_ext_1.txt", np.mean(acq_ext_1, axis = 0), delimiter = "\n")
	np.savetxt("R/plot_acq/decoupled/txts/acq_ext_2.txt", np.mean(acq_ext_2, axis = 0), delimiter = "\n")
	np.savetxt("R/plot_acq/decoupled/txts/acq_ext_3.txt", np.mean(acq_ext_3, axis = 0), delimiter = "\n")
	np.savetxt("R/plot_acq/decoupled/txts/acq_ap_1.txt", np.mean(acq_ap_1, axis = 0), delimiter = "\n")
	np.savetxt("R/plot_acq/decoupled/txts/acq_ap_2.txt", np.mean(acq_ap_2, axis = 0), delimiter = "\n")
	np.savetxt("R/plot_acq/decoupled/txts/acq_ap_3.txt", np.mean(acq_ap_3, axis = 0), delimiter = "\n")
...
def test_plot_results(num_results):
	...
	np.savetxt("R/plot_acq/coupled/txts/acq_ext.txt", np.mean(acq_ext, axis = 0), delimiter = "\n")
	np.savetxt("R/plot_acq/coupled/txts/acq_ap.txt", np.mean(acq_ap, axis = 0), delimiter = "\n")
	import pdb; pdb.set_trace()
