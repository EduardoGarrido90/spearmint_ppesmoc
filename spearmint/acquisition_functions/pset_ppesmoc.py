...
############################################################################################
def _build_set_of_points_that_conditions_GPs_old(obj_models, con_models, all_tasks, all_constraints, pareto_set):
    X = pareto_set
    for t in all_tasks:
...
#This function assigns jitter to every black box function.
def assign_jitter(obj_models, con_models):
    jitter = dict()
...
def ep(obj_models, pareto_set, minimize=True, con_models=[], input_space=None, Xtest=None):
    ...
    jitter = assign_jitter(obj_models, con_models)
...
def updateMarginals(a):
	...
	for cons in all_constraints:
		...
		vTilde_cons[ np.eye(n_total).astype('bool') ] = np.append(np.sum(a['a_c_hfhat'][ :, : , ntask ], axis = 1), \
		...
		mTilde_cons = np.append(np.sum(a['b_c_hfhat'][ :, : , ntask ], axis = 1), \
	...
	for obj in all_tasks:
		...
		#vTilde[ np.eye(n_total).astype('bool') ][ n_obs , :] = np.append(np.sum(a['ahfhat'][ :, : , ntask, 1, 1 ], axis = 0) + \
		#	np.sum(a['chfhat'][ :, : , ntask, 0, 0 ], axis = 1) + np.sum(a['chfhat'][ :, : , ntask, 1, 1 ], axis = 0),  \
		#	np.sum(a['ahfhat'][ :, : , ntask, 0, 0 ], axis = 1))
		vTilde[ np.eye(n_total).astype('bool') ] = np.append(np.sum(a['ahfhat'][ :, : , ntask, 0, 0 ], axis = 1), \
		...
		vTilde[ n_obs : n_total, n_obs : n_total ] = vTilde[ n_obs : n_total, n_obs : n_total ] + \
			a['chfhat'][ :, : , ntask, 0, 1 ] + a['chfhat'][ :, : , ntask, 1, 0 ].T

		#vTilde[ n_pset : n_total, 0 : n_pset ] = a['ahfhat'][ :, :, ntask, 0, 1]
		vTilde[ 0 : n_obs, n_obs : n_total ] = a['ahfhat'][ :, :, ntask, 0, 1]
		#vTilde[ 0 : n_pset, n_pset : n_total ] =  a['ahfhat'][ :, :, ntask, 0, 1].transpose()
		vTilde[ n_obs : n_total, 0 : n_obs ] =  a['ahfhat'][ :, :, ntask, 0, 1].transpose()
		...
		mTilde = np.append(np.sum(a['bhfhat'][ :, : , ntask, 0 ], axis = 1), 
			np.sum(a['bhfhat'][ :, : , ntask, 1 ], axis = 0) + \
			np.sum(a['dhfhat'][ :, : , ntask, 0 ], axis = 1) + np.sum(a['dhfhat'][ :, : , ntask, 1 ], axis = 0))
...
def update_full_Factors(a, damping, minimize=True, no_negative_variances_nor_nands = False, no_negatives = True):
    ...
    n_task = 0
    for obj in all_tasks:
        mNew_pset[ n_task, :, : ] = np.repeat(a['m'][ obj ][n_obs : n_obs + n_pset], n_test).reshape(((n_pset, n_test)))
        mNew_test[ n_task, :, : ] = np.repeat(a['m'][ obj ][n_obs + n_pset : n_total], n_pset).reshape(((n_test, n_pset))).transpose()
        vNew_pset[ n_task, :, : ] = np.repeat(np.diag(a['V'][ obj ])[ n_obs : n_obs + n_pset ], n_test).reshape(((n_pset, n_test)))
        vNew_test[ n_task, :, : ] = np.repeat(np.diag(a['V'][ obj ])[ n_obs + n_pset : n_total ], n_pset).reshape(((n_test, n_pset))).transpose()
        vNew_cov[ n_task, :, : ] = a['V'][ obj ][ n_obs : n_obs + n_pset, n_obs + n_pset : n_total ]
        covOrig[ n_task, :, : ] = a['V'][ obj ][ n_obs : n_obs + n_pset, n_obs + n_pset : n_total ]
        n_task += 1

    n_task = 0
    for cons in all_constraints:
        mNew_test_cons[ n_task, :, : ] = np.outer(np.ones(n_pset),  a['m_cons'][ cons ][ n_obs + n_pset : n_total ])
        vNew_test_cons[ n_task, :, : ] = np.outer(np.ones(n_pset), np.diag(a['V_cons'][ cons ])[n_obs + n_pset : n_total])
        n_task += 1
    ...
    a['g_c_hfhat'][:, :, :] = vTilde_cons_new.T * damping + (1 - damping) * a['g_c_hfhat'][:, :, :]
    vTilde_cons = a['g_c_hfhat'].T
    a['h_c_hfhat'][:, :, :] = mTilde_cons_new.T * damping + (1 - damping) * a['h_c_hfhat'][:, :, :]
    mTilde_cons = a['h_c_hfhat'].T
...
##########################################################################################################
# predictEP_multiple_iter_optim_robust: Computes the EP factors that depend on the candidate factor creating
# the final conditional predictive distribution. Computes the predictive distributions. but the chol of the kernel matrix and the
# chol of the test matrix are already provided. Method that approximates the predictive distribution at a particular location.
#
# INPUT:
#
# obj_models: GPs that contain the objectives.
# con_models: GPs that contain the constraints.
# a: Data structure with CPDs, PDs and EP factors. Conditioned on the non dependant of the candidate factors.
# pareto_set: Pareto set sample.
# Xtest: Candidate/s.
# damping: Damping factor.
# n_iters: Number of iterations of the EP, it does not go until convergence.
# Other control flagas.
# OUTPUT:
#  {'mf': mfNew, 'vf':vfNew, 'mfo': mf, 'vfo': vf, 'mc': mcNew, 'vc': vcNew, 'mco': mc, 'vco': vc}: Conditional Predictive Distributions.
#
# TODOs:
# DONE 1. Disable loop until convergence. 
# DONE 2. mf, vf, mc and vc: Where are they computed? In compute_full_unconstrained_predictive_distribution. Must they be set to old structures? No.
# Hence, we comment in a the old structures, as they are no longer used.
# 3. Set the matrices and means without the test points in this method for the EP factors non dependant on X.
# 3. Test for casual PESMOC.
# 4. Put instead of one candidate a batch of candidates selected at random from above.
# 5. Modify the block matrix structure for a batch.
# 6. Tests for PPESMOC.
##########################################################################################################
...
def predictEP_multiple_iter_optim_robust(obj_models, con_models, a, no_negatives = True, minimize=True, damping=.1):
    ...
    Kstar, mf, mc, mP, mP_cons, mPset, mPset_cons, vf, vc, vP, vP_cons, cov, vPset, vPset_cons, Xtest_ext = \
		compute_full_unconstrained_predictive_distribution(a, n_pset, a['X_test'], all_tasks, n_test, obj_models, all_constraints)
    ...
    ##########################################################
    #NOT ADAPTED FROM HERE TO THE JOINED METHOD OF THE UPDATES
    ##########################################################
    ...
    #######################################################################################
    #WORTH TO SEE FOR THE METHOD OF COMPUTING THE GRADIENT OF PPESMOC. Watch PPESMOC notes.
    #######################################################################################

    n_task = 0
    # http://www.cs.nthu.edu.tw/~jang/book/addenda/matinv/matinv/
    # In PESMOC it was special case 1.
    # Here, it is the General Formula: Matrix Inversion in Block form. 
    for obj in all_tasks:
        A = vOld_full_pset[ obj ]
        Ainv = matrixInverse(vOld_full_pset[ obj ])
    ...
    a['V_final'] = vfNew
    #a['V_final_old'] = vf
    a['m_cons_final'] = mcNew
    #a['m_cons_final_old'] = mc
    a['V_cons_final'] = vcNew
    #a['V_cons_final_old'] = vc
...
class PPESMOC(AbstractAcquisitionFunction):
	...
	#################################################################################################
	# compute_ep_solution_for_not_dependant_on_candidate_factors
	#
	# INPUT:
	...
	#
	# OUTPUT:
	# epSolution: Dictionary with int(self.options['pesm_samples_per_hyper']) EP approximations 
	# of the factors that do not depend on the candidate and other useful information.
	#################################################################################################
	def compute_ep_solution_for_not_dependant_on_candidate_factors(self, pareto_set, obj_model_dict, minimize, con_models_dict, key, Xtest):
                        ...
                        for i in range(int(self.options['pesm_samples_per_hyper'])):
                                if pareto_set[ str(i) ] is None:
                                        ...
                                        epSolution[ str(i) ] = ep(obj_model_dict, pareto_set[ str(i) ], minimize=minimize, \
                                                con_models=con_models_dict, input_space=self.input_space, Xtest=Xtest)
	...
	def acquisition(self, obj_model_dict, con_models_dict, cand, current_best, compute_grad, minimize=True, tasks=None, tasks_values=None):
		...
		models = obj_models
		#self.assert_preconditions(models, compute_grad) #Compute preconditions for PPESMOC.
		self.verify_models_same_state_and_grad(obj_models, con_models_dict.values(), compute_grad)
		...
		pareto_set = self.sample_pareto_sets(models, con_models_dict, key) #We sample a dictionary of Pareto Sets.
		...
		acq_dict, omegas = self.compute_ep(pareto_set, obj_model_dict, minimize, con_models_dict, key, cand)
		...
		print "Computing gradients of PPESMOC acquisition function"
		acq_grad = self.compute_gradients_wrt_test_points(obj_model_dict, con_models_dict, pareto_set, tasks, cand, omegas)

		print "Returning acquisition and gradients"
		print total_acq
		print acq_grad
		return total_acq, acq_grad


	def compute_gradients_wrt_test_points_optimized(self, obj_model_dict, con_model_dict, pareto_set, tasks, cand, D):
		return 

	EPSILON = 1e-8
	def compute_gradients_wrt_test_points(self, obj_model_dict, con_model_dict, pareto_set, tasks, cand, Omega):
		if cand.ndim == 1:
                        cand = cand[None]
		PPESMOC.all_tasks = obj_model_dict
		PPESMOC.all_constraints = con_model_dict

		acq_grad = np.zeros((len(obj_model_dict) + len(con_model_dict), cand.shape[0], cand.shape[1])) 
		acq_grad_test = np.zeros((len(obj_model_dict) + len(con_model_dict), cand.shape[0], cand.shape[1])) 
		unconstrained_variances_derivative = dict()
		constrained_variances_derivative = dict()
		obj_con_dict = obj_model_dict.copy()
		obj_con_dict.update(con_model_dict)
		testing_covariances_matrices = None	###TESTING
		for i in range(int(self.options['pesm_samples_per_hyper'])):
                        if pareto_set[ str(i) ] is None:
                                continue;
                        else:
                		covariances_matrices = self.get_covariances_matrices(obj_model_dict, con_model_dict, pareto_set[str(i)], cand, \
											Omega[str(i)])
				testing_covariances_matrices = covariances_matrices ###TESTING
				n_task = 0
				for obj in obj_con_dict:
					unconstrained_variances_derivative[obj] = dict()
					constrained_variances_derivative[obj] = dict()
					covariances_matrices_task = self.cov_and_blocks_task(covariances_matrices, obj)
					n_test_point=0
					for test_point in cand: 
						n_dim=0
						unconstrained_variances_derivative[obj][str(n_test_point)] = dict()
						constrained_variances_derivative[obj][str(n_test_point)] = dict()
						for dim_test_point in test_point:
							acq_grad[n_task, n_test_point, n_dim] += np.trace(np.dot(covariances_matrices_task["inv_Byy"], self.dev_Byy_xij(covariances_matrices_task, obj_con_dict[obj], n_test_point, n_dim))) - np.trace(np.dot(covariances_matrices_task["inv_Eyy"], self.dev_Eyy_xij(covariances_matrices_task, obj_con_dict[obj], n_test_point, n_dim)))
							unconstrained_variances_derivative[obj][str(n_test_point)][str(n_dim)] = self.dev_Byy_xij(covariances_matrices_task, obj_con_dict[obj], n_test_point, n_dim)
							constrained_variances_derivative[obj][str(n_test_point)][str(n_dim)] = self.dev_Eyy_xij(covariances_matrices_task, obj_con_dict[obj], n_test_point, n_dim)
							n_dim+=1
						n_test_point+=1
					n_task+=1

                acq_grad /= float(self.options['pesm_samples_per_hyper'])

                if np.any(np.isnan(acq_grad)):
                	raise Exception("Acquisition function containts NaN for task %s" % t)

                total_grad_acq = np.zeros((cand.shape[0], cand.shape[1]))
		n_task = 0
                for task in tasks:
                        total_grad_acq += acq_grad[ n_task, :, : ]
			n_task += 1
	
		###TESTING, IN PROD RETURN ONLY TOTAL_GRAD_ACQ
		return total_grad_acq, acq_grad, unconstrained_variances_derivative, constrained_variances_derivative, testing_covariances_matrices

	covariances_matrices_test_task = None
	def dev_Kbt_xij(self, covariances_matrices, tp, dp):
		index_zero_entries = np.array([x for x in np.arange(covariances_matrices['n_test']) if x != tp])
		dev_matrix = np.copy(covariances_matrices['dev_kbt_xij'][:, :, dp])
		dev_matrix[:, index_zero_entries] = 0.0
		return dev_matrix 
		#return (PPESMOC.covariances_matrices_test_task["Kbt"]-covariances_matrices["Kbt"])/PPESMOC.EPSILON

	def dev_Kot_xij(self, covariances_matrices, tp, dp):
		index_zero_entries = np.array([x for x in np.arange(covariances_matrices['n_test']) if x != tp])
		dev_matrix = np.copy(covariances_matrices['dev_kot_xij'][:, :, dp])
		dev_matrix[:, index_zero_entries] = 0.0
		return dev_matrix 
		#return (PPESMOC.covariances_matrices_test_task["Kot"]-covariances_matrices["Kot"])/PPESMOC.EPSILON

	#Not sure if it would be better to compute (- partial Kbt / partial xij) here.
	#The minus is because the gradient wrt inputs_1 = -inputs_2. I do not think that it is a good idea.
	def dev_Ktb_xij(self, covariances_matrices, tp, dp):
		index_zero_entries = np.array([x for x in np.arange(covariances_matrices['n_test']) if x != tp])
                dev_matrix = np.copy(covariances_matrices['dev_ktb_xij'][:, :, dp])
                dev_matrix[index_zero_entries, :] = 0.0
                return dev_matrix
		#return (PPESMOC.covariances_matrices_test_task["Ktb"]-covariances_matrices["Ktb"])/PPESMOC.EPSILON

	def dev_Kto_xij(self, covariances_matrices, tp, dp):
		index_zero_entries = np.array([x for x in np.arange(covariances_matrices['n_test']) if x != tp])
                dev_matrix = np.copy(covariances_matrices['dev_kto_xij'][:, :, dp])
                dev_matrix[index_zero_entries, :] = 0.0
                return dev_matrix	
		#return (PPESMOC.covariances_matrices_test_task["Kto"]-covariances_matrices["Kto"])/PPESMOC.EPSILON

	#Here there is a doubt, I am obtaining the gradient wrt to a test point. As these are the covs between the test point
	#then, the point tp affects its row and column. Then, the zeros are only the rest of the matrix which is neither the
	#row or the column of tp. I have considered this, it may not be correct.
	def dev_Ktt_xij(self, covariances_matrices, tp, dp):
		index_zero_entries = np.array([x for x in np.arange(covariances_matrices['n_test']) if x != tp])
                dev_matrix = np.copy(covariances_matrices['dev_ktt_xij'][:, :, dp])
		final_matrix = np.zeros((covariances_matrices['n_test'],covariances_matrices['n_test']))
		final_matrix[tp] = dev_matrix[tp]*(-1)
		#final_matrix[:,tp] = dev_matrix[:,tp]*(-1)
		final_matrix[:,tp] = dev_matrix[:,tp]
                return final_matrix
		#return (PPESMOC.covariances_matrices_test_task["Ktt"]-covariances_matrices["Ktt"])/PPESMOC.EPSILON

	
	'''
	def dev_Eyy_xij(self, covariances_matrices, task_model, tp, dp):
		return np.dot(np.dot(- covariances_matrices['Eyy'], ( self.dev_par_Byy_minus_Byx_Bxx_inv_Bxy_par_inv(covariances_matrices, task_model, tp, dp) - ( \
				np.dot(np.dot(self.dev_minus_Byy_inv_Byx_par_Bxx_minus_Bxy_Byy_inv_Byx_par_inv(covariances_matrices, task_model, tp, dp), matrixInverse(covariances_matrices['Dxx'])), \
				covariances_matrices['Dxy']) + np.dot(np.dot(covariances_matrices['Dyx'], (np.dot(np.dot(- matrixInverse(covariances_matrices['Dxx']), \
				self.dev_par_Bxx_minus_Bxy_Byy_inv_Byx_par_inv(covariances_matrices, task_model, tp, dp)), matrixInverse(covariances_matrices['Dxx'])))), \
				covariances_matrices['Dxy']) + np.dot(matrixInverse(covariances_matrices['Dxx']), \
				self.dev_minus_Bxx_inv_Bxy_par_Byy_minus_Byx_Bxx_inv_Bxy_par_inv(covariances_matrices, task_model, tp, dp))))), covariances_matrices['Eyy'])
	'''

	#def dev_Eyy_xij_in_K_function(self, covariances_matrices, task_model, tp, dp):
		#It is almost impossible to translate the expression member by member to the code without commiting several errors.
		#Mainly because the np.dots.
		#So first I compute blocks, in order to reduce the complexity.
		
		#return np.dot(- covariances_matrices['Eyy'], np.dot(- (covariances_matrices['Byy'] - np.dot(np.dot(covariances_matrices['Byx'], matrixInverse(covariances_matrices['Bxx'])), matrixInverse(covariances_matrices['Bxy']))), (self.dev_)


	#FOR TESTING def dev_Eyy_xij(self, covariances_matrices, covariances_matrices_test_task, task_model, tp, dp):
	def dev_Eyy_xij_KO(self, covariances_matrices, task_model, tp, dp):
		#self.test_derivatives(covariances_matrices, covariances_matrices_test_task, task_model, tp, dp)
		return np.dot(np.dot(-covariances_matrices['Eyy'], self.dev_Dyy_xij(covariances_matrices, task_model, tp, dp) - \
			np.dot(np.dot(self.dev_Dyx_xij(covariances_matrices, task_model, tp, dp), matrixInverse(covariances_matrices['Dxx'])),\
			covariances_matrices['Dxy']) + np.dot(covariances_matrices['Dyx'], np.dot(np.dot(np.dot(-matrixInverse(\
			covariances_matrices['Dxx']), self.dev_Dxx_xij(covariances_matrices, task_model, tp, dp)),matrixInverse(\
			covariances_matrices['Dxx'])), covariances_matrices['Dxy']) + np.dot(matrixInverse(covariances_matrices['Dxx']),\
			self.dev_Dxy_xij(covariances_matrices, task_model, tp, dp)))), covariances_matrices['Eyy'])

	def dev_Eyy_xij(self, covariances_matrices, task_model, tp, dp):
                #self.test_derivatives(covariances_matrices, covariances_matrices_test_task, task_model, tp, dp)
                return np.dot(np.dot(-covariances_matrices['Eyy'], self.dev_Dyy_xij(covariances_matrices, task_model, tp, dp) - \
		np.dot(np.dot(self.dev_Dyx_xij(covariances_matrices, task_model, tp, dp),matrixInverse(covariances_matrices['Dxx'])),covariances_matrices['Dxy']) + np.dot(covariances_matrices['Dyx'],np.dot(matrixInverse(covariances_matrices['Dxx']),np.dot(self.dev_Dxx_xij(covariances_matrices, task_model, tp, dp),np.dot(matrixInverse(covariances_matrices['Dxx']),covariances_matrices['Dxy'])))) - np.dot(covariances_matrices['Dyx'],np.dot(matrixInverse(covariances_matrices['Dxx']),self.dev_Dxy_xij(covariances_matrices, task_model, tp, dp)))), covariances_matrices['Eyy'])

	all_tasks = None
	all_constraints = None
	def test_derivatives(self, covariances_matrices, covariances_matrices_test_task, task_model, tp, dp):
		PPESMOC.covariances_matrices_test_task = covariances_matrices_test_task
		import pdb; pdb.set_trace();
		print "###################### INITIATING TEST BENCH #######################"
		print "# TEST 1: B_yy"
		print "Derivative:"
		print self.dev_Byy_xij(covariances_matrices_test_task, task_model, tp, dp)
		print "Test derivative"
		print (covariances_matrices_test_task["Byy"]-covariances_matrices["Byy"])/PPESMOC.EPSILON
		print "# TEST 2: B_yx"
                print "Derivative:"
                print self.dev_Byx_xij(covariances_matrices, task_model, tp, dp)
                print "Test derivative"
                print (covariances_matrices_test_task["Byx"]-covariances_matrices["Byx"])/PPESMOC.EPSILON
		print "# TEST 3: B_xy"
                print "Derivative:"
                print self.dev_Bxy_xij(covariances_matrices, task_model, tp, dp)
                print "Test derivative"
                print (covariances_matrices_test_task["Bxy"]-covariances_matrices["Bxy"])/PPESMOC.EPSILON
		print "# TEST 4: B_xx"
                print "Derivative:"
                print self.dev_Bxx_xij(covariances_matrices, task_model, tp, dp)
                print "Test derivative"
                print (covariances_matrices_test_task["Bxx"]-covariances_matrices["Bxx"])/PPESMOC.EPSILON
		print "# TEST 5: frac{partial D_xy}{x_ij} = np.transpose(frac{partial D_yx}{x_ij})"
                print "frac{partial D_xy}{x_ij}"
                print self.dev_Dxy_xij(covariances_matrices, task_model, tp, dp)
                print "np.transpose(frac{partial D_yx}{x_ij})"
		print np.transpose(self.dev_Dyx_xij(covariances_matrices, task_model, tp, dp))
		print "# TEST 6: D_yy"
                print "Derivative:"
                print self.dev_Dyy_xij(covariances_matrices, task_model, tp, dp)
                print "Test derivative"
                print (covariances_matrices_test_task["Dyy"]-covariances_matrices["Dyy"])/PPESMOC.EPSILON
                print "# TEST 7: D_yx"
                print "Derivative:"
                print self.dev_Dyx_xij(covariances_matrices, task_model, tp, dp)
                print "Test derivative"
                print (covariances_matrices_test_task["Dyx"]-covariances_matrices["Dyx"])/PPESMOC.EPSILON
                print "# TEST 8: D_xy"
                print "Derivative:"
                print self.dev_Dxy_xij(covariances_matrices, task_model, tp, dp)
                print "Test derivative"
                print (covariances_matrices_test_task["Dxy"]-covariances_matrices["Dxy"])/PPESMOC.EPSILON
                print "# TEST 9: D_xx"
                print "Derivative:"
                print self.dev_Dxx_xij(covariances_matrices, task_model, tp, dp)
                print "Test derivative"
                print (covariances_matrices_test_task["Dxx"]-covariances_matrices["Dxx"])/PPESMOC.EPSILON	
		print "# TEST 10: E_yy"
                print "Derivative:"
                print self.dev_Eyy_xij(covariances_matrices, task_model, tp, dp)
                print "Test derivative"
                print (covariances_matrices_test_task["Eyy"]-covariances_matrices["Eyy"])/PPESMOC.EPSILON
		print "# TEST 11: Full expression"
		print "Full expression"
               	print  np.trace(np.dot(covariances_matrices["inv_Eyy"], self.dev_Eyy_xij(covariances_matrices, task_model, tp,dp)))
                print "Test full expression"
                print (np.trace(np.dot(covariances_matrices_test_task["inv_Eyy"], self.dev_Eyy_xij(covariances_matrices_test_task, task_model, tp, dp))) - np.trace(np.dot(covariances_matrices["inv_Eyy"], self.dev_Eyy_xij(covariances_matrices, task_model, tp, dp))))/ PPESMOC.EPSILON
		print "# TEST 11B: Full expression"
                print "Full expression"
                print  np.trace(np.dot(covariances_matrices["inv_Eyy"], self.dev_Eyy_xij(covariances_matrices, task_model, tp,dp)))
                print "Test full expression"
		print  np.trace(np.dot(covariances_matrices["inv_Eyy"], (covariances_matrices_test_task["Eyy"]-covariances_matrices["Eyy"])/PPESMOC.EPSILON))
		#Poner la funcion objetivo aqui en vez del 11B, caso 11C.
                # f(x) = log|Eyy|.
                # f'(x) = tr(Eyy^(-1)*dev(Eyy,xij)).
		print "# TEST 11C: Full expression"
                print "Full expression"
		print  np.trace(np.dot(covariances_matrices["inv_Eyy"], self.dev_Eyy_xij(covariances_matrices, task_model, tp,dp)))
		print "Test full expression"
		print (np.log(np.linalg.det(covariances_matrices_test_task["Eyy"])) - np.log(np.linalg.det(covariances_matrices["Eyy"])))/PPESMOC.EPSILON
		import pdb; pdb.set_trace();
		print "# TEST 12: K_tt"
                print "Derivative:"
                print self.dev_Ktt_xij(covariances_matrices_test_task, tp, dp)
                print "Test derivative"
                print (covariances_matrices_test_task["Ktt"]-covariances_matrices["Ktt"])/PPESMOC.EPSILON
                print "# TEST 13: K_ot"
                print "Derivative:"
                print self.dev_Kot_xij(covariances_matrices, tp, dp)
                print "Test derivative"
                print (covariances_matrices_test_task["Kot"]-covariances_matrices["Kot"])/PPESMOC.EPSILON
                print "# TEST 14: K_to"
                print "Derivative:"
                print self.dev_Kto_xij(covariances_matrices, tp, dp)
                print "Test derivative"
                print (covariances_matrices_test_task["Kto"]-covariances_matrices["Kto"])/PPESMOC.EPSILON
                print "# TEST 15: K_bt"
                print "Derivative:"
                print self.dev_Kbt_xij(covariances_matrices, tp, dp)
                print "Test derivative"
                print (covariances_matrices_test_task["Kbt"]-covariances_matrices["Kbt"])/PPESMOC.EPSILON
		print "# TEST 16: K_tb"
                print "Derivative:"
                print self.dev_Ktb_xij(covariances_matrices, tp, dp)
                print "Test derivative"
                print (covariances_matrices_test_task["Ktb"]-covariances_matrices["Ktb"])/PPESMOC.EPSILON
		print "# TEST 17: K_ob"
                print "Derivative: must be zero."
                print "Test derivative"
                print (covariances_matrices_test_task["Kob"]-covariances_matrices["Kob"])/PPESMOC.EPSILON
		print "# TEST 18: K_bo"
                print "Derivative must be zero:"
                print "Test derivative"
                print (covariances_matrices_test_task["Kbo"]-covariances_matrices["Kbo"])/PPESMOC.EPSILON
		print "#####################################################################"
		import pdb; pdb.set_trace();
		return

	def dev_Dxy_xij(self, covariances_matrices, task_model, tp, dp):
		return np.dot(matrixInverse(covariances_matrices["Bxx"]),np.dot(self.dev_Bxx_xij(covariances_matrices, task_model, tp, dp),np.dot(matrixInverse(covariances_matrices["Bxx"]),np.dot(covariances_matrices["Bxy"],covariances_matrices["Cyy"])))) - np.dot(matrixInverse(covariances_matrices["Bxx"]),np.dot(self.dev_Bxy_xij(covariances_matrices, task_model, tp, dp),covariances_matrices["Cyy"])) - np.dot(matrixInverse(covariances_matrices["Bxx"]),np.dot(covariances_matrices["Bxy"],self.dev_Dyy_xij(covariances_matrices, task_model, tp, dp)))

	def dev_Dyx_xij(self, covariances_matrices, task_model, tp, dp):
                return np.dot(matrixInverse(covariances_matrices["Byy"]),np.dot(self.dev_Byy_xij(covariances_matrices, task_model, tp, dp),np.dot(matrixInverse(covariances_matrices["Byy"]),np.dot(covariances_matrices["Byx"],covariances_matrices["Cxx"])))) - np.dot(matrixInverse(covariances_matrices["Byy"]),np.dot(self.dev_Byx_xij(covariances_matrices, task_model, tp, dp),covariances_matrices["Cxx"])) - np.dot(matrixInverse(covariances_matrices["Byy"]),np.dot(covariances_matrices["Byx"],self.dev_Dxx_xij(covariances_matrices, task_model, tp, dp)))

	def dev_Dyy_xij(self, covariances_matrices, task_model, tp, dp):
                return - np.dot(covariances_matrices["Cyy"],np.dot(self.dev_Byy_xij(covariances_matrices, task_model, tp, dp) - np.dot(self.dev_Byx_xij(covariances_matrices, task_model, tp, dp),np.dot(matrixInverse(covariances_matrices["Bxx"]),covariances_matrices["Bxy"])) + np.dot(covariances_matrices["Byx"],np.dot(matrixInverse(covariances_matrices["Bxx"]),np.dot(self.dev_Bxx_xij(covariances_matrices, task_model, tp, dp),np.dot(matrixInverse(covariances_matrices["Bxx"]),covariances_matrices["Bxy"]))))- np.dot(np.dot(covariances_matrices["Byx"],matrixInverse(covariances_matrices["Bxx"])),self.dev_Bxy_xij(covariances_matrices, task_model, tp, dp) ) ,covariances_matrices["Cyy"]))

	def dev_Dxx_xij(self, covariances_matrices, task_model, tp, dp):
                return - np.dot(covariances_matrices["Cxx"],np.dot(self.dev_Bxx_xij(covariances_matrices, task_model, tp, dp) - np.dot(self.dev_Bxy_xij(covariances_matrices, task_model, tp, dp),np.dot(matrixInverse(covariances_matrices["Byy"]),covariances_matrices["Byx"])) + np.dot(covariances_matrices["Bxy"],np.dot(matrixInverse(covariances_matrices["Byy"]),np.dot(self.dev_Byy_xij(covariances_matrices, task_model, tp, dp),np.dot(matrixInverse(covariances_matrices["Byy"]),covariances_matrices["Byx"]))))- np.dot(np.dot(covariances_matrices["Bxy"],matrixInverse(covariances_matrices["Byy"])),self.dev_Byx_xij(covariances_matrices, task_model, tp, dp) ) ,covariances_matrices["Cxx"]))

	def dev_Dxy_xij_complicated(self, covariances_matrices, task_model, tp, dp):
		return np.dot(np.dot(np.dot(np.dot(matrixInverse(covariances_matrices["Bxx"]),\
                        -np.zeros(covariances_matrices["Bxx"].shape)), -matrixInverse(covariances_matrices["Bxx"])),covariances_matrices["Bxy"]) - \
                np.dot(matrixInverse(covariances_matrices["Bxx"]),\
                self.dev_Kbt_xij(covariances_matrices, tp, dp) - np.dot(np.dot(covariances_matrices["Kbo"], \
                        covariances_matrices["inv_noisied_Koo"]), self.dev_Kot_xij(covariances_matrices, tp, dp))), matrixInverse(covariances_matrices["Byy"] - np.dot(np.dot(covariances_matrices["Byx"],matrixInverse(covariances_matrices["Bxx"])), covariances_matrices["Bxy"]))) -\
			np.dot(np.dot(-matrixInverse(covariances_matrices["Bxx"]),covariances_matrices["Bxy"]),\
			np.dot(-matrixInverse(covariances_matrices["Byy"] - np.dot(np.dot(covariances_matrices["Byx"], \
                        matrixInverse(covariances_matrices["Bxx"])), covariances_matrices["Bxy"])), np.dot(\
                        np.dot(-matrixInverse(covariances_matrices["Byy"]),np.dot( self.dev_Ktt_xij(covariances_matrices, tp, dp) - np.dot(np.dot(\
                        self.dev_Kto_xij(covariances_matrices, tp, dp), covariances_matrices['inv_noisied_Koo']), \
                        self.dev_Kot_xij(covariances_matrices, tp, dp)),covariances_matrices["Byy"]))\
                         - np.dot(np.dot(self.dev_Ktb_xij(covariances_matrices, tp, dp) - \
                        np.dot(np.dot(self.dev_Kto_xij(covariances_matrices, tp, dp), covariances_matrices["inv_noisied_Koo"]), \
                        covariances_matrices["Kob"]), matrixInverse(covariances_matrices["Bxx"])) + \
                        np.dot(covariances_matrices["Byx"], np.zeros(covariances_matrices["Bxx"].shape)), covariances_matrices["Bxy"]) + \
                        np.dot(np.dot(covariances_matrices["Byx"],matrixInverse(covariances_matrices["Bxx"])), \
                        self.dev_Kbt_xij(covariances_matrices, tp, dp) - np.dot(np.dot(covariances_matrices["Kbo"], \
                        covariances_matrices["inv_noisied_Koo"]), self.dev_Kot_xij(covariances_matrices, tp, dp))) ,\
                        matrixInverse(covariances_matrices["Byy"] - np.dot(np.dot(covariances_matrices["Byx"], \
                        matrixInverse(covariances_matrices["Bxx"])), covariances_matrices["Bxy"])))))

	def dev_Dxy_xij_b(self, covariances_matrices, task_model, tp, dp):
                return np.dot(np.dot(np.dot(np.dot(matrixInverse(covariances_matrices["Bxx"]),\
                        -np.zeros(covariances_matrices["Bxx"].shape)), -matrixInverse(covariances_matrices["Bxx"])),covariances_matrices["Bxy"]) - \
                np.dot(matrixInverse(covariances_matrices["Bxx"]),\
                self.dev_Kbt_xij(covariances_matrices, tp, dp) - np.dot(np.dot(covariances_matrices["Kbo"], \
                        covariances_matrices["inv_noisied_Koo"]), self.dev_Kot_xij(covariances_matrices, tp, dp))), covariances_matrices["Cyy"]) -\
                        np.dot(np.dot(-matrixInverse(covariances_matrices["Bxx"]),covariances_matrices["Bxy"]),\
                        self.dev_Dyy_xij(covariances_matrices, task_model, tp, dp))

	def dev_Dyx_xij_complicated(self, covariances_matrices, task_model, tp, dp):
		return np.dot(np.dot(np.dot(np.dot(matrixInverse(covariances_matrices["Byy"]),self.dev_Ktt_xij(covariances_matrices, tp, dp) - \
                        np.dot(np.dot(self.dev_Kto_xij(covariances_matrices, tp, dp), covariances_matrices['inv_noisied_Koo']), \
                        self.dev_Kot_xij(covariances_matrices, tp, dp))),-matrixInverse(covariances_matrices["Byy"])), \
                        covariances_matrices["Byx"]) - np.dot(matrixInverse(covariances_matrices["Byy"]),\
                        self.dev_Ktb_xij(covariances_matrices, tp, dp) - np.dot(np.dot(self.dev_Kto_xij(covariances_matrices, tp, dp), \
                        covariances_matrices["inv_noisied_Koo"]), covariances_matrices["Kob"])),\
			matrixInverse(covariances_matrices["Bxx"] - np.dot(np.dot(covariances_matrices["Bxy"],\
                        matrixInverse(covariances_matrices["Byy"])),covariances_matrices["Byx"]))) - np.dot(\
			np.dot(matrixInverse(covariances_matrices["Byy"]),covariances_matrices["Byx"]),\
			np.dot(-matrixInverse(covariances_matrices["Bxx"] - np.dot(np.dot(covariances_matrices["Bxy"], \
                       matrixInverse(covariances_matrices["Byy"])), covariances_matrices["Byx"])), \
                       np.dot(np.zeros(covariances_matrices["Bxx"].shape) - np.dot(np.dot(self.dev_Kbt_xij(covariances_matrices, tp, dp) - \
                       np.dot(np.dot(covariances_matrices["Kbo"], \
                        covariances_matrices["inv_noisied_Koo"]), self.dev_Kot_xij(covariances_matrices, tp, dp)), \
                        matrixInverse(covariances_matrices["Byy"])) + np.dot(covariances_matrices["Bxy"], np.dot(np.dot(\
                        -matrixInverse(covariances_matrices["Byy"]),self.dev_Ktt_xij(covariances_matrices, tp, dp) - np.dot(np.dot(\
                        self.dev_Kto_xij(covariances_matrices, tp, dp), covariances_matrices['inv_noisied_Koo']), \
                        self.dev_Kot_xij(covariances_matrices, tp, dp))),matrixInverse(covariances_matrices["Byy"]))),covariances_matrices["Byx"]) + \
                        np.dot(np.dot(covariances_matrices["Bxy"],matrixInverse(covariances_matrices["Byy"])),\
                        self.dev_Ktb_xij(covariances_matrices, tp, dp) - np.dot(np.dot(self.dev_Kto_xij(covariances_matrices, tp, dp), \
                        covariances_matrices["inv_noisied_Koo"]), covariances_matrices["Kob"])), matrixInverse(covariances_matrices["Bxx"] \
                        - np.dot(np.dot(covariances_matrices["Bxy"], matrixInverse(covariances_matrices["Byy"])), covariances_matrices["Byx"])))))

	def dev_Dyx_xij_b(self, covariances_matrices, task_model, tp, dp):
                return np.dot(np.dot(np.dot(np.dot(matrixInverse(covariances_matrices["Byy"]),self.dev_Ktt_xij(covariances_matrices, tp, dp) - \
                        np.dot(np.dot(self.dev_Kto_xij(covariances_matrices, tp, dp), covariances_matrices['inv_noisied_Koo']), \
                        self.dev_Kot_xij(covariances_matrices, tp, dp))),-matrixInverse(covariances_matrices["Byy"])), \
                        covariances_matrices["Byx"]) - np.dot(matrixInverse(covariances_matrices["Byy"]),\
                        self.dev_Ktb_xij(covariances_matrices, tp, dp) - np.dot(np.dot(self.dev_Kto_xij(covariances_matrices, tp, dp), \
                        covariances_matrices["inv_noisied_Koo"]), covariances_matrices["Kob"])),\
                        covariances_matrices["Cxx"]) - np.dot(\
                        np.dot(matrixInverse(covariances_matrices["Byy"]),covariances_matrices["Byx"]), \
			self.dev_Dxx_xij(covariances_matrices, task_model, tp, dp))

	def dev_Dxx_xij_complicated(self, covariances_matrices, task_model, tp, dp):
		return np.dot(-matrixInverse(covariances_matrices["Bxx"] - np.dot(np.dot(covariances_matrices["Bxy"], \
                       matrixInverse(covariances_matrices["Byy"])), covariances_matrices["Byx"])), \
		       np.dot(np.zeros(covariances_matrices["Bxx"].shape) - np.dot(np.dot(self.dev_Kbt_xij(covariances_matrices, tp, dp) - \
		       np.dot(np.dot(covariances_matrices["Kbo"], \
                        covariances_matrices["inv_noisied_Koo"]), self.dev_Kot_xij(covariances_matrices, tp, dp)), \
	...
	def dev_Dxx_xij_b(self, covariances_matrices, task_model, tp, dp):
                return np.dot(-covariances_matrices["Cxx"], \
                       np.dot(np.zeros(covariances_matrices["Bxx"].shape) - np.dot(np.dot(self.dev_Kbt_xij(covariances_matrices, tp, dp) - \
                       np.dot(np.dot(covariances_matrices["Kbo"], \
                        covariances_matrices["inv_noisied_Koo"]), self.dev_Kot_xij(covariances_matrices, tp, dp)), \
                        matrixInverse(covariances_matrices["Byy"])) + np.dot(covariances_matrices["Bxy"], np.dot(np.dot(\
                        ...
                        np.dot(np.dot(covariances_matrices["Bxy"],matrixInverse(covariances_matrices["Byy"])),\
                        self.dev_Ktb_xij(covariances_matrices, tp, dp) - np.dot(np.dot(self.dev_Kto_xij(covariances_matrices, tp, dp), \
                        covariances_matrices["inv_noisied_Koo"]), covariances_matrices["Kob"])), covariances_matrices["Cxx"]))

	def dev_Dyy_xij_complicated(self, covariances_matrices, task_model, tp, dp):
		return np.dot(-matrixInverse(covariances_matrices["Byy"] - np.dot(np.dot(covariances_matrices["Byx"], \
                        matrixInverse(covariances_matrices["Bxx"])), covariances_matrices["Bxy"])), np.dot(\
			np.dot(-matrixInverse(covariances_matrices["Byy"]),np.dot( self.dev_Ktt_xij(covariances_matrices, tp, dp) - np.dot(np.dot(\
                        self.dev_Kto_xij(covariances_matrices, tp, dp), covariances_matrices['inv_noisied_Koo']), \
                        self.dev_Kot_xij(covariances_matrices, tp, dp)),covariances_matrices["Byy"]))\
	...
	def dev_Dyy_xij_b(self, covariances_matrices, task_model, tp, dp):
                return np.dot(-covariances_matrices["Cyy"], np.dot(\
                        np.dot(-matrixInverse(covariances_matrices["Byy"]),np.dot( self.dev_Ktt_xij(covariances_matrices, tp, dp) - np.dot(np.dot(\
                        self.dev_Kto_xij(covariances_matrices, tp, dp), covariances_matrices['inv_noisied_Koo']), \
                        self.dev_Kot_xij(covariances_matrices, tp, dp)),covariances_matrices["Byy"]))\
                         - np.dot(np.dot(self.dev_Ktb_xij(covariances_matrices, tp, dp) - \
	...
	#Modularizar esto, comprobar que funciona y volver a montar entero.
	def dev_Dyy_xij_KO(self, covariances_matrices, task_model, tp, dp):
		return np.dot(-matrixInverse(covariances_matrices["Byy"] - np.dot(np.dot(covariances_matrices["Byx"], \
                        matrixInverse(covariances_matrices["Bxx"])), covariances_matrices["Bxy"])),\
			(self.dev_Ktt_xij(covariances_matrices, tp, dp) - np.dot(np.dot(\
			self.dev_Kto_xij(covariances_matrices, tp, dp), covariances_matrices['inv_noisied_Koo']), \
			self.dev_Kot_xij(covariances_matrices, tp, dp))) - \

			#b1 = np.dot(self.dev_Ktb_xij(covariances_matrices, tp, dp) -np.dot(np.dot(self.dev_Kto_xij(covariances_matrices, tp, dp), covariances_matrices["inv_noisied_Koo"]),covariances_matrices["Kob"]), matrixInverse(covariances_matrices["Bxx"]))
			#b2 = np.dot(covariances_matrices["Byx"],np.dot(-matrixInverse(covariances_matrices["Bxx"]),np.dot(np.zeros(covariances_matrices["Bxx"].shape), matrixInverse(covariances_matrices["Bxx"]))))
			#b3 = np.dot(b1+b2,covariances_matrices["Bxy"])
			#b4 = np.dot(np.dot(covariances_matrices["Byx"],matrixInverse(covariances_matrices["Bxx"])), self.dev_Kbt_xij(covariances_matrices, tp, dp) -np.dot(np.dot(covariances_matrices["Kbo"],covariances_matrices["inv_noisied_Koo"]), self.dev_Kot_xij(covariances_matrices, tp, dp)))
			#b5 = self.dev_Ktt_xij(covariances_matrices, tp, dp) - np.dot(np.dot(self.dev_Kto_xij(covariances_matrices, tp, dp), covariances_matrices['inv_noisied_Koo']),self.dev_Kot_xij(covariances_matrices, tp, dp))
			#b5-b3*b4 ¿¿¿???

			(np.dot(np.dot(self.dev_Ktb_xij(covariances_matrices, tp, dp) - \
                        np.dot(np.dot(self.dev_Kto_xij(covariances_matrices, tp, dp), covariances_matrices["inv_noisied_Koo"]), \
                        covariances_matrices["Kob"]), matrixInverse(covariances_matrices["Bxx"]))+ \
                        np.dot(covariances_matrices["Byx"], np.dot(-matrixInverse(covariances_matrices["Bxx"]), \
			np.dot(np.zeros(covariances_matrices["Bxx"].shape), matrixInverse(covariances_matrices["Bxx"])))), \
                        covariances_matrices["Bxy"]) +\

			np.dot(np.dot(covariances_matrices["Byx"],\
                        matrixInverse(covariances_matrices["Bxx"])), self.dev_Kbt_xij(covariances_matrices, tp, dp) - \
                        np.dot(np.dot(covariances_matrices["Kbo"], \
                        covariances_matrices["inv_noisied_Koo"]), self.dev_Kot_xij(covariances_matrices, tp, dp)))),\
	...
	def dev_Eyy_xij_old(self, covariances_matrices, task_model, tp, dp):
		return np.dot(np.dot(- covariances_matrices['Eyy'], self.dev_Eyy_xij_2(covariances_matrices, task_model, tp, dp)), covariances_matrices['Eyy'])

	def dev_Eyy_xij_2(self, covariances_matrices, task_model, tp, dp):
		return self.dev_par_Byy_minus_Byx_Bxx_inv_Bxy_par_inv(covariances_matrices, task_model, tp, dp) - self.dev_Eyy_xij_3(covariances_matrices, task_model, tp, dp)

	def dev_Eyy_xij_3(self, covariances_matrices, task_model, tp, dp):
		return np.dot(self.dev_minus_Byy_inv_Byx_par_Bxx_minus_Bxy_Byy_inv_Byx_par_inv(covariances_matrices, task_model, tp, dp),
			covariances_matrices['inv_Dxx_Dxy']) + self.dev_Eyy_xij_4(covariances_matrices, task_model, tp, dp) 
	
	def dev_Eyy_xij_4(self, covariances_matrices, task_model, tp, dp):
		return np.dot(covariances_matrices['Dyx'], self.dev_Eyy_xij_5(covariances_matrices, task_model, tp, dp))

	def dev_Eyy_xij_5(self, covariances_matrices, task_model, tp, dp):
		return self.dev_Eyy_xij_6(covariances_matrices, task_model, tp, dp) + self.dev_Eyy_xij_7(covariances_matrices, task_model, tp, dp)

	def dev_Eyy_xij_6(self, covariances_matrices, task_model, tp, dp):
		return np.dot(self.dev_Eyy_xij_8(covariances_matrices, task_model, tp, dp), covariances_matrices['Dxy'])

	def dev_Eyy_xij_7(self, covariances_matrices, task_model, tp, dp):
		return np.dot(covariances_matrices['inv_Dxx'], self.dev_minus_Bxx_inv_Bxy_par_Byy_minus_Byx_Bxx_inv_Bxy_par_inv(covariances_matrices, task_model, tp, dp))

	def dev_Eyy_xij_8(self, covariances_matrices, task_model, tp, dp):
		return np.dot(np.dot(-covariances_matrices['inv_Dxx'], self.dev_par_Bxx_minus_Bxy_Byy_inv_Byx_par_inv(covariances_matrices, task_model, tp, dp)), covariances_matrices['inv_Dxx'])

	def dev_par_Bxx_minus_Bxy_Byy_inv_Byx_par_inv(self, covariances_matrices, task_model, tp, dp):
                return np.dot(np.dot(-matrixInverse(covariances_matrices['Bxx'] - covariances_matrices['Bxy_inv_Byy_Byx']), \
                        self.dev_Bxx_minus_Bxy_Byy_inv_Byx(covariances_matrices, task_model, tp, dp)), \
                        matrixInverse(covariances_matrices['Bxx'] - covariances_matrices['Bxy_inv_Byy_Byx'])) 

        def dev_Bxx_minus_Bxy_Byy_inv_Byx(self, covariances_matrices, task_model, tp, dp):
                return self.dev_Bxx_xij(covariances_matrices, task_model, tp, dp) - self.dev_minus_Bxy_Byy_inv_Byx(covariances_matrices, task_model, tp, dp)

        def dev_minus_Bxy_Byy_inv_Byx(self, covariances_matrices, task_model, tp, dp):
                return np.dot(self.dev_Bxy_Byy_inv(covariances_matrices, task_model, tp, dp), covariances_matrices['Byx']) - \
                       np.dot(covariances_matrices['Bxy_inv_Byy'], self.dev_Byx_xij(covariances_matrices, task_model, tp, dp))

        def dev_Bxy_Byy_inv(self, covariances_matrices, task_model, tp, dp):
                return np.dot(self.dev_Bxy_xij(covariances_matrices, task_model, tp, dp), covariances_matrices['inv_Byy']) - \
                        np.dot(covariances_matrices['Bxy'], self.dev_Byy_inv(covariances_matrices, task_model, tp, dp))

        def dev_Byy_inv(self, covariances_matrices, task_model, tp, dp):
                return np.dot(np.dot(-covariances_matrices['inv_Byy'], self.dev_Byy_xij(covariances_matrices, task_model, tp, dp)), covariances_matrices['inv_Byy'])

	def dev_minus_Bxx_inv_Bxy_par_Byy_minus_Byx_Bxx_inv_Bxy_par_inv(self, covariances_matrices, task_model, tp, dp):
                return np.dot(self.dev_minus_Bxx_inv_Bxy(covariances_matrices, task_model, tp, dp), matrixInverse(covariances_matrices['Byy'] -
                        covariances_matrices['Byx_inv_Bxx_Bxy']))  - np.dot(covariances_matrices['inv_Bxx_Bxy'], \
                        self.dev_par_Byy_minus_Byx_Bxx_inv_Bxy_par_inv(covariances_matrices, task_model, tp, dp))

	def dev_minus_Bxx_inv_Bxy(self, covariances_matrices, task_model, tp, dp):
		return np.dot(self.dev_minus_Bxx_inv(covariances_matrices, task_model, tp, dp), covariances_matrices['Bxy']) - np.dot(covariances_matrices['inv_Bxx'], \
			( - self.dev_Bxy_xij(covariances_matrices, task_model, tp, dp)))
	
	def dev_minus_Bxx_inv(self, covariances_matrices, task_model, tp, dp):
		return np.dot(np.dot(covariances_matrices['inv_Bxx'], (-self.dev_Bxx_xij(covariances_matrices, task_model, tp, dp))),(-covariances_matrices['inv_Bxx'])) 

	def dev_minus_Byy_inv_Byx_par_Bxx_minus_Bxy_Byy_inv_Byx_par_inv(self, covariances_matrices, task_model, tp, dp):
		return np.dot(self.dev_minus_Byy_inv_Byx(covariances_matrices, task_model, tp, dp), matrixInverse(covariances_matrices['Bxx'] - \
			covariances_matrices['Bxy_inv_Byy_Byx']))  - np.dot(covariances_matrices['inv_Byy_Byx'], \
			self.dev_par_Bxx_minus_Bxy_Byy_inv_Byx_par_inv(covariances_matrices, task_model, tp, dp))

	def dev_minus_Byy_inv_Byx(self, covariances_matrices, task_model, tp, dp):
		return np.dot(self.dev_minus_Byy_inv(covariances_matrices, task_model, tp, dp), covariances_matrices['Byx']) - np.dot(covariances_matrices['inv_Byy'], self.dev_Byx_xij(covariances_matrices, task_model, tp, dp))

	def dev_minus_Byy_inv(self, covariances_matrices, task_model, tp, dp):
                return np.dot(np.dot(covariances_matrices['inv_Byy'], self.dev_Byy_xij(covariances_matrices, task_model, tp, dp)), (-covariances_matrices['inv_Byy']))

	def dev_par_Byy_minus_Byx_Bxx_inv_Bxy_par_inv(self, covariances_matrices, task_model, tp, dp):
		return np.dot(np.dot(-matrixInverse( covariances_matrices['Byy'] - covariances_matrices['Byx_inv_Bxx_Bxy']), \
			self.dev_Byy_minus_Byx_Bxx_inv_Bxy(covariances_matrices, task_model, tp, dp)), \
			matrixInverse( covariances_matrices['Byy'] - covariances_matrices['Byx_inv_Bxx_Bxy'])) 

	def dev_Byy_minus_Byx_Bxx_inv_Bxy(self, covariances_matrices, task_model, tp, dp):
		return self.dev_Byy_xij(covariances_matrices, task_model, tp, dp) - self.dev_minus_Byx_Bxx_inv_Bxy(covariances_matrices, task_model, tp, dp)

	def dev_minus_Byx_Bxx_inv_Bxy(self, covariances_matrices, task_model, tp, dp):
		return np.dot(self.dev_minus_Byx_Bxx_inv(covariances_matrices, task_model, tp, dp), covariances_matrices['Bxy']) - \
			np.dot(covariances_matrices['Byx_inv_Bxx'], self.dev_Bxy_xij(covariances_matrices, task_model, tp, dp))

	def dev_minus_Byx_Bxx_inv(self, covariances_matrices, task_model, tp, dp):
		return np.dot(-self.dev_Byx_xij(covariances_matrices, task_model, tp, dp), covariances_matrices['inv_Bxx']) - \
			np.dot(covariances_matrices['Byx'], self.dev_Bxx_inv(covariances_matrices, task_model, tp, dp))

	#Shape inv_Bxx: n*n, dev_Bxx_xij: n*n.
	def dev_Bxx_inv(self, covariances_matrices, task_model, tp, dp):
		return np.dot(np.dot(-covariances_matrices['inv_Bxx'], self.dev_Bxx_xij(covariances_matrices, task_model, tp, dp)), covariances_matrices['inv_Bxx'])

	def dev_Bxx_xij(self, covariances_matrices, task_model, tp, dp):
		return np.zeros(covariances_matrices['Bxx'].shape)

	def dev_Bxx_xij_alternative(self, covariances_matrices, task_model, tp, dp):
		return covariances_matrices['Kbb'] - np.dot(np.dot(covariances_matrices['Kbo'],covariances_matrices['inv_noisied_Koo']),covariances_matrices['Kob'])

	def dev_Bxy_xij(self, covariances_matrices, task_model, tp, dp):
		return self.dev_Kbt_xij(covariances_matrices, tp, dp) - np.dot(np.dot(covariances_matrices["Kbo"] ,matrixInverse(covariances_matrices['Koo'] + task_model.noise_value())), self.dev_Kot_xij(covariances_matrices, tp, dp))
	
	def dev_Byx_xij(self, covariances_matrices, task_model, tp, dp):
                return self.dev_Ktb_xij(covariances_matrices, tp, dp) - np.dot(np.dot(self.dev_Kto_xij(covariances_matrices, tp, dp) ,matrixInverse(covariances_matrices['Koo'] + task_model.noise_value())), covariances_matrices["Kob"])

	def dev_Byy_xij(self, covariances_matrices, task_model, tp, dp):
                return self.dev_Ktt_xij(covariances_matrices, tp, dp) - np.dot(np.dot(self.dev_Kto_xij(covariances_matrices, tp, dp), covariances_matrices['inv_noisied_Koo']), covariances_matrices['Kot']) - np.dot(np.dot(covariances_matrices['Kto'], covariances_matrices['inv_noisied_Koo']), self.dev_Kot_xij(covariances_matrices, tp, dp))

	def cov_mat_for_task(self, covariances_matrices, task, Omegas):
		covariances_matrices_task = dict()
		omegas_task = dict()

		omegas_task['omega_xx'] = Omegas['omega_xx'][task]
		omegas_task['omega_xy'] = Omegas['omega_xy'][task]
		omegas_task['omega_yx'] = Omegas['omega_yx'][task]
		omegas_task['omega_yy'] = Omegas['omega_yy'][task]

		covariances_matrices_task['X'] = covariances_matrices['X']
		covariances_matrices_task['n_obs'] = covariances_matrices['n_obs']
		covariances_matrices_task['n_pset'] = covariances_matrices['n_pset']
		covariances_matrices_task['n_test'] = covariances_matrices['n_test']
		covariances_matrices_task['n_total'] = covariances_matrices['n_total']
		'''
		covariances_matrices_task['V_obs'] = covariances_matrices['V_obs'][task]
		covariances_matrices_task['V_par'] = covariances_matrices['V_par'][task]
		covariances_matrices_task['V_test'] = covariances_matrices['V_test'][task]
		covariances_matrices_task['V'] = covariances_matrices['V'][task]
		'''
		covariances_matrices_task['Koo'] = covariances_matrices['Koo'][task]
		covariances_matrices_task['Kob'] = covariances_matrices['Kob'][task]
		covariances_matrices_task['Kot'] = covariances_matrices['Kot'][task]
		covariances_matrices_task['Kbb'] = covariances_matrices['Kbb'][task]
		covariances_matrices_task['Kbo'] = covariances_matrices['Kbo'][task]
		covariances_matrices_task['Kbt'] = covariances_matrices['Kbt'][task]
		covariances_matrices_task['Ktt'] = covariances_matrices['Ktt'][task]
		covariances_matrices_task['Ktb'] = covariances_matrices['Ktb'][task]
		covariances_matrices_task['Kto'] = covariances_matrices['Kto'][task]
		covariances_matrices_task['dev_kot_xij'] = covariances_matrices['dev_kot_xij'][task]
		covariances_matrices_task['dev_kto_xij'] = covariances_matrices['dev_kto_xij'][task]
		covariances_matrices_task['dev_kbt_xij'] = covariances_matrices['dev_kbt_xij'][task]
		covariances_matrices_task['dev_ktb_xij'] = covariances_matrices['dev_ktb_xij'][task]
		covariances_matrices_task['dev_ktt_xij'] = covariances_matrices['dev_ktt_xij'][task]
		return covariances_matrices_task, omegas_task

	def cov_and_blocks_task(self, covariances_matrices, task):
		covariances_matrices_task = dict()
                covariances_matrices_task['X'] = covariances_matrices['X']
                covariances_matrices_task['n_obs'] = covariances_matrices['n_obs']
                covariances_matrices_task['n_pset'] = covariances_matrices['n_pset']
		...
		'''
                ...
                covariances_matrices_task['dev_ktb_xij'] = covariances_matrices['dev_ktb_xij'][task]
                covariances_matrices_task['dev_ktt_xij'] = covariances_matrices['dev_ktt_xij'][task]
                covariances_matrices_task['Bxx'] = covariances_matrices['Bxx'][task]
                covariances_matrices_task['Bxy'] = covariances_matrices['Bxy'][task]
                covariances_matrices_task['Byx'] = covariances_matrices['Byx'][task]
                covariances_matrices_task['Byy'] = covariances_matrices['Byy'][task]
                covariances_matrices_task['Cxx'] = covariances_matrices['Cxx'][task]
                covariances_matrices_task['Cxy'] = covariances_matrices['Cxy'][task]
                covariances_matrices_task['Cyx'] = covariances_matrices['Cyx'][task]
                covariances_matrices_task['Cyy'] = covariances_matrices['Cyy'][task]
                covariances_matrices_task['Dxx'] = covariances_matrices['Dxx'][task]
                covariances_matrices_task['Dxy'] = covariances_matrices['Dxy'][task]
                covariances_matrices_task['Dyx'] = covariances_matrices['Dyx'][task]
                covariances_matrices_task['Dyy'] = covariances_matrices['Dyy'][task]
                covariances_matrices_task['Exx'] = covariances_matrices['Exx'][task]
                covariances_matrices_task['Exy'] = covariances_matrices['Exy'][task]
                covariances_matrices_task['Eyx'] = covariances_matrices['Eyx'][task]
                covariances_matrices_task['Eyy'] = covariances_matrices['Eyy'][task]
		covariances_matrices_task['inv_Dxx'] = covariances_matrices['inv_Dxx'][task]
                covariances_matrices_task['inv_Dxx_Dxy'] = covariances_matrices['inv_Dxx_Dxy'][task]
                covariances_matrices_task['inv_Byy'] = covariances_matrices['inv_Byy'][task]
                covariances_matrices_task['Bxy_inv_Byy'] = covariances_matrices['Bxy_inv_Byy'][task]
                covariances_matrices_task['Bxy_inv_Byy_Byx'] = covariances_matrices['Bxy_inv_Byy_Byx'][task]
                covariances_matrices_task['inv_Bxx'] = covariances_matrices['inv_Bxx'][task]
                covariances_matrices_task['inv_Bxx_Bxy'] = covariances_matrices['inv_Bxx_Bxy'][task]
                covariances_matrices_task['Byx_inv_Bxx'] = covariances_matrices['Byx_inv_Bxx'][task]
                covariances_matrices_task['Byx_inv_Bxx_Bxy'] = covariances_matrices['Byx_inv_Bxx_Bxy'][task]
                covariances_matrices_task['inv_Byy_Byx'] = covariances_matrices['inv_Byy_Byx'][task]
                covariances_matrices_task['inv_noisied_Koo'] = covariances_matrices['inv_noisied_Koo'][task]
                covariances_matrices_task['inv_Koo_Kob'] = covariances_matrices['inv_Koo_Kob'][task]
                covariances_matrices_task['Kbo_inv_Koo'] = covariances_matrices['Kbo_inv_Koo'][task]
                covariances_matrices_task['inv_Eyy'] = covariances_matrices['inv_Eyy'][task]
                return covariances_matrices_task
		
	#TODO TODO TODO D -> Omegas.
	#TODO QUESTION. Caution: I am sure that predict is not the way to extract the variance. Shapes fit, but I am not sure.
	#Study this fact. At least for observations. For pareto set and test points this MIGHT be the way.
	#CAUTION 2: There are multiple objectives and constraints, this is not shown in PPESMOC notes.
	def get_covariances_matrices(self, obj_model_dict, con_model_dict, pareto_set, cand, Omegas):
		all_tasks = obj_model_dict.copy()
                all_constraints= con_model_dict.copy()
		X, n_obs, n_pset, n_test, n_total = build_set_of_points_that_conditions_GPs(obj_model_dict, con_model_dict, pareto_set, cand)
		#Debug for extracting values needed for derivatives.
		#X[n_obs + n_pset : n_total][1,1] += 1e-08
		#V_obs = dict()
		#V_par = dict()
		#V_test = dict()
		#V = dict()
		...
		dev_ktb_xij = dict()
		dev_ktt_xij = dict()
		Dxx = dict() # x index is o + b, y is t.
		Dxy = dict()
		Dyx = dict()
		Dyy = dict()
		#import pdb; pdb.set_trace();
		obj_con_dict = all_tasks.copy()
		obj_con_dict.update(all_constraints)
		for t in obj_con_dict:
			Koo[t] = obj_con_dict[t].noiseless_kernel.cross_cov(X[0 : n_obs],X[0 : n_obs])
                        Kob[t] = obj_con_dict[t].noiseless_kernel.cross_cov(X[0 : n_obs],X[n_obs : n_obs + n_pset])
                        Kot[t] = obj_con_dict[t].noiseless_kernel.cross_cov(X[0 : n_obs],X[n_obs + n_pset : n_total])
                        dev_kot_xij[t] = obj_con_dict[t].noiseless_kernel.cross_cov_grad_data(X[0 : n_obs], X[n_obs + n_pset : n_total])
                        Kbo[t] = obj_con_dict[t].noiseless_kernel.cross_cov(X[n_obs : n_obs + n_pset], X[0 : n_obs])
                        Kbt[t] = obj_con_dict[t].noiseless_kernel.cross_cov(X[n_obs : n_obs + n_pset], X[n_obs + n_pset : n_total])
                        dev_kbt_xij[t] = obj_con_dict[t].noiseless_kernel.cross_cov_grad_data(X[n_obs : n_obs + n_pset], X[n_obs + n_pset : n_total])
                        Kbb[t] = obj_con_dict[t].noiseless_kernel.cross_cov(X[n_obs : n_obs + n_pset], X[n_obs : n_obs + n_pset])
                        dev_kbt_xij[t] = obj_con_dict[t].noiseless_kernel.cross_cov_grad_data(X[n_obs : n_obs + n_pset], X[n_obs + n_pset : n_total])
                        Kto[t] = obj_con_dict[t].noiseless_kernel.cross_cov(X[n_obs + n_pset : n_total], X[0: n_obs])
                        dev_kto_xij[t] = - obj_con_dict[t].noiseless_kernel.cross_cov_grad_data(X[n_obs + n_pset : n_total], X[0 : n_obs])
                        Ktb[t] = obj_con_dict[t].noiseless_kernel.cross_cov(X[n_obs + n_pset : n_total],  X[n_obs : n_obs + n_pset])
                        dev_ktb_xij[t] = - obj_con_dict[t].noiseless_kernel.cross_cov_grad_data(X[n_obs + n_pset : n_total], X[n_obs : n_obs + n_pset])
                        Ktt[t] = obj_con_dict[t].noiseless_kernel.cross_cov(X[n_obs + n_pset : n_total],  X[n_obs + n_pset : n_total])
			dev_ktt_xij[t] =  obj_con_dict[t].noiseless_kernel.cross_cov_grad_data(X[n_obs + n_pset : n_total], X[n_obs + n_pset : n_total])
		covariances_matrices = {
			'X' : X, 'n_obs' : n_obs, 'n_pset' : n_pset, 'n_test' : n_test, 'n_total' : n_total,
			'Koo' : Koo, 'Kob' : Kob, 'Kot' : Kot, 'Kbb' : Kbb, 'Kbo' : Kbo, 'Kbt' : Kbt, 'Ktt' : Ktt, 'Ktb' : Ktb, 'Kto' : Kto,
			'dev_kot_xij' : dev_kot_xij, 'dev_kbt_xij' : dev_kbt_xij, 'dev_kto_xij' : dev_kto_xij, 'dev_ktb_xij' : dev_ktb_xij, 'dev_ktt_xij': dev_ktt_xij 
		}
		covariances_matrices = self.compute_blocks(covariances_matrices, Omegas, all_tasks, all_constraints, obj_model_dict, con_model_dict)
		covariances_matrices['inv_Dxx'] = dict()
		covariances_matrices['inv_Dxx_Dxy'] = dict()
		covariances_matrices['inv_Byy'] = dict()
		covariances_matrices['Bxy_inv_Byy'] = dict()
		covariances_matrices['Bxy_inv_Byy_Byx'] = dict()
		covariances_matrices['inv_Bxx'] = dict()
		covariances_matrices['inv_Bxx_Bxy'] = dict()
		covariances_matrices['Byx_inv_Bxx'] = dict()
		covariances_matrices['Byx_inv_Bxx_Bxy'] = dict()
		covariances_matrices['inv_Byy_Byx'] = dict()
		covariances_matrices['inv_noisied_Koo'] = dict()
		covariances_matrices['inv_Koo_Kob'] = dict()
		covariances_matrices['Kbo_inv_Koo'] = dict()
		covariances_matrices['inv_Eyy'] = dict()
		for t in obj_con_dict:
			covariances_matrices = self.precompute_matrices_for_dev(covariances_matrices, t, obj_con_dict[t])
		return covariances_matrices

	def precompute_matrices_for_dev(self, covariances_matrices, task, task_model):
		covariances_matrices['inv_Dxx'][task] = matrixInverse(covariances_matrices['Dxx'][task])
		covariances_matrices['inv_Dxx_Dxy'][task] = np.dot(matrixInverse(covariances_matrices['Dxx'][task]), covariances_matrices['Dxy'][task])
		covariances_matrices['inv_Byy'][task] = matrixInverse(covariances_matrices['Byy'][task])
		covariances_matrices['Bxy_inv_Byy'][task] = np.dot(covariances_matrices['Bxy'][task], matrixInverse(covariances_matrices['Byy'][task]))
		covariances_matrices['Bxy_inv_Byy_Byx'][task] = np.dot(np.dot(covariances_matrices['Bxy'][task], matrixInverse(covariances_matrices['Byy'][task])), covariances_matrices['Byx'][task])
		covariances_matrices['inv_Bxx'][task] = matrixInverse(covariances_matrices['Bxx'][task])
		covariances_matrices['inv_Bxx_Bxy'][task] = np.dot(matrixInverse(covariances_matrices['Bxx'][task]), covariances_matrices['Bxy'][task])
		covariances_matrices['Byx_inv_Bxx'][task] = np.dot(covariances_matrices['Byx'][task], matrixInverse(covariances_matrices['Bxx'][task]))
		covariances_matrices['Byx_inv_Bxx_Bxy'][task] = np.dot(np.dot(covariances_matrices['Byx'][task], matrixInverse(covariances_matrices['Bxx'][task])),covariances_matrices['Bxy'][task])
		covariances_matrices['inv_Byy_Byx'][task] = np.dot(matrixInverse(covariances_matrices['Byy'][task]), covariances_matrices['Byx'][task])
		noisy_koo = np.copy(covariances_matrices['Koo'][task])
                np.fill_diagonal(noisy_koo, np.diagonal(covariances_matrices['Koo'][task]) + task_model.noise_value())
		covariances_matrices['inv_noisied_Koo'][task] = matrixInverse(noisy_koo)
		covariances_matrices['inv_Koo_Kob'][task] = np.dot(noisy_koo, covariances_matrices['Kob'][task])
		covariances_matrices['Kbo_inv_Koo'][task] = np.dot(covariances_matrices['Kbo'][task], noisy_koo)	
		covariances_matrices['inv_Eyy'][task] = matrixInverse(covariances_matrices['Eyy'][task])
		return covariances_matrices
	
	def compute_blocks(self, covariances_matrices, Omegas, all_tasks, all_constraints, obj_model_dict, con_model_dict):
		covariances_matrices = self.init_covariances_matrices_for_blocks(covariances_matrices)
		n_task = 0
		obj_con_dict = all_tasks.copy()
		obj_con_dict.update(all_constraints)
		for obj in obj_con_dict:
			covariances_matrices_for_task, omegas_task = self.cov_mat_for_task(covariances_matrices, obj, Omegas)
			covariances_matrices_for_task = self.compute_B_blocks(covariances_matrices_for_task, obj_con_dict[obj])
			covariances_matrices_for_task = self.compute_C_blocks(covariances_matrices_for_task)
			covariances_matrices_for_task = self.compute_D_blocks(covariances_matrices_for_task, omegas_task)
			covariances_matrices_for_task = self.compute_E_blocks(covariances_matrices_for_task)
			covariances_matrices = self.fill_covariances_matrices_with_blocks(covariances_matrices, covariances_matrices_for_task, obj)
			n_task = n_task + 1
		return covariances_matrices

	#As this only requires NPSET, IT IS OK. But the reality is that it should inclue observations matrices
	def compute_B_blocks_only_npset(self, covariances_matrices, task_model):
                noisy_koo = np.copy(covariances_matrices['Koo'])
                np.fill_diagonal(noisy_koo, np.diagonal(covariances_matrices['Koo']) + task_model.noise_value())
                covariances_matrices['Bxx'] = covariances_matrices['Kbb'] - np.dot(np.dot(covariances_matrices['Kbo'], \
                       matrixInverse(noisy_koo)), covariances_matrices['Kob'])
                covariances_matrices['Bxy'] = covariances_matrices['Kbt'] - np.dot(np.dot(covariances_matrices['Kbo'], \
                        matrixInverse(noisy_koo)), covariances_matrices['Kot']) 
                covariances_matrices['Byx'] = covariances_matrices['Ktb'] - np.dot(np.dot(covariances_matrices['Kto'], \
                        matrixInverse(noisy_koo)), covariances_matrices['Kob'])
                covariances_matrices['Byy'] = covariances_matrices['Ktt'] - np.dot(np.dot(covariances_matrices['Kto'], \
                        matrixInverse(noisy_koo)), covariances_matrices['Kot'])
                return covariances_matrices

	#Can be heavily optimized due to the fact that it computes lots of matrix inverses.
	#ATTENTION ULTRA GIGA PETA DANGER: X IS N_PSET+N_OBS, MATRICES SHOULD BE BUILT.
	def compute_B_blocks(self, covariances_matrices, task_model):
		noisy_koo = np.copy(covariances_matrices['Koo'])
		np.fill_diagonal(noisy_koo, np.diagonal(covariances_matrices['Koo']) + task_model.noise_value())
		noisy_kbb = np.copy(covariances_matrices['Kbb'])
                np.fill_diagonal(noisy_kbb, np.diagonal(covariances_matrices['Kbb']) + task_model.noise_value())
		#covariances_matrices['Bxx'] = covariances_matrices['Kbb'] - np.dot(np.dot(covariances_matrices['Kbo'], \
		#	matrixInverse(noisy_koo)), covariances_matrices['Kob'])
		B_bb = covariances_matrices['Kbb'] - np.dot(np.dot(covariances_matrices['Kbo'], \
                       matrixInverse(noisy_koo)), covariances_matrices['Kob'])
		#B_oo = 
		import pdb; pdb.set_trace();
		covariances_matrices['Bxy'] = covariances_matrices['Kbt'] - np.dot(np.dot(covariances_matrices['Kbo'], \
	...
	def compute_C_blocks(self, covariances_matrices):
		covariances_matrices['Cxx'] = matrixInverse(covariances_matrices['Bxx'] - np.dot(np.dot(covariances_matrices['Bxy'], \
			matrixInverse(covariances_matrices['Byy'])), covariances_matrices['Byx']))
		covariances_matrices['Cxy'] = - np.dot(np.dot(matrixInverse(covariances_matrices['Bxx']), covariances_matrices['Bxy']), matrixInverse( \
			covariances_matrices['Byy'] - np.dot(np.dot(covariances_matrices['Byx'], matrixInverse(covariances_matrices['Bxx'])), \
			covariances_matrices['Bxy'])))
		covariances_matrices['Cyx'] = - np.dot(np.dot(matrixInverse(covariances_matrices['Byy']), covariances_matrices['Byx']), matrixInverse( \
                        covariances_matrices['Bxx'] - np.dot(np.dot(covariances_matrices['Bxy'], matrixInverse(covariances_matrices['Byy'])), \
                        covariances_matrices['Byx'])))
		covariances_matrices['Cyy'] = matrixInverse(covariances_matrices['Byy'] - np.dot(np.dot(covariances_matrices['Byx'], \
                        matrixInverse(covariances_matrices['Bxx'])), covariances_matrices['Bxy']))
                return covariances_matrices

	#The gradient of the observations is zero, these covariances matrices are between the pareto set points and the test points?
	#I do not have it clear!!!! But I think that it is the source of all the problems. Check out with Daniel the indexes.
	def compute_D_blocks(self, covariances_matrices, Omegas):
		n_obs = covariances_matrices['n_obs']
		n_pset = covariances_matrices['n_pset']
		n_total = covariances_matrices['n_total']
		covariances_matrices['Dxx'] = covariances_matrices['Cxx'] + Omegas['omega_xx'][n_obs : n_obs+n_pset, n_obs : n_obs+n_pset]
		covariances_matrices['Dxy'] = covariances_matrices['Cxy'] + Omegas['omega_xy'][n_obs : n_obs+n_pset,]
		covariances_matrices['Dyx'] = covariances_matrices['Cyx'] + Omegas['omega_yx'][:,n_obs : n_obs+n_pset]
		covariances_matrices['Dyy'] = covariances_matrices['Cyy'] + Omegas['omega_yy']
		#EP_FACTORS=0
		#covariances_matrices['Dxx'] = covariances_matrices['Cxx'] 
		#covariances_matrices['Dxy'] = covariances_matrices['Cxy'] 
		#covariances_matrices['Dyx'] = covariances_matrices['Cyx'] 
		#covariances_matrices['Dyy'] = covariances_matrices['Cyy']
                return covariances_matrices

	def compute_E_blocks(self, covariances_matrices):
		covariances_matrices['Exx'] = matrixInverse(covariances_matrices['Dxx'] - np.dot(np.dot(covariances_matrices['Dxy'], \
                        matrixInverse(covariances_matrices['Dyy'])), covariances_matrices['Dyx']))
                covariances_matrices['Exy'] = - np.dot(np.dot(matrixInverse(covariances_matrices['Dxx']), covariances_matrices['Dxy']), matrixInverse( \
                        covariances_matrices['Dyy'] - np.dot(np.dot(covariances_matrices['Dyx'], matrixInverse(covariances_matrices['Dxx'])), \
                        covariances_matrices['Dxy'])))
                covariances_matrices['Eyx'] = - np.dot(np.dot(matrixInverse(covariances_matrices['Dyy']), covariances_matrices['Dyx']), matrixInverse( \
                        covariances_matrices['Dxx'] - np.dot(np.dot(covariances_matrices['Dxy'], matrixInverse(covariances_matrices['Dyy'])), \
                        covariances_matrices['Dyx'])))
                covariances_matrices['Eyy'] = matrixInverse(covariances_matrices['Dyy'] - np.dot(np.dot(covariances_matrices['Dyx'], \
                        matrixInverse(covariances_matrices['Dxx'])), covariances_matrices['Dxy']))
                return covariances_matrices

	def init_covariances_matrices_for_blocks(self, covariances_matrices):
		covariances_matrices['Bxx'] = dict()
		covariances_matrices['Bxy'] = dict()
		covariances_matrices['Byx'] = dict()
		covariances_matrices['Byy'] = dict()
		covariances_matrices['Cxx'] = dict()
		covariances_matrices['Cxy'] = dict()
		covariances_matrices['Cyx'] = dict()
		covariances_matrices['Cyy'] = dict()
		covariances_matrices['Dxx'] = dict()
		covariances_matrices['Dxy'] = dict()
		covariances_matrices['Dyx'] = dict()
		covariances_matrices['Dyy'] = dict()
		covariances_matrices['Exx'] = dict()
		covariances_matrices['Exy'] = dict()
		covariances_matrices['Eyx'] = dict()
		covariances_matrices['Eyy'] = dict()
		return covariances_matrices


	def fill_covariances_matrices_with_blocks(self, covariances_matrices, covariances_matrices_for_task, task):
		covariances_matrices['Bxx'][task] = covariances_matrices_for_task['Bxx']	
		covariances_matrices['Bxy'][task] = covariances_matrices_for_task['Bxy']	
		covariances_matrices['Byx'][task] = covariances_matrices_for_task['Byx']	
		covariances_matrices['Byy'][task] = covariances_matrices_for_task['Byy']	
		covariances_matrices['Cxx'][task] = covariances_matrices_for_task['Cxx']	
		covariances_matrices['Cxy'][task] = covariances_matrices_for_task['Cxy']	
		covariances_matrices['Cyx'][task] = covariances_matrices_for_task['Cyx']	
		covariances_matrices['Cyy'][task] = covariances_matrices_for_task['Cyy']	
		covariances_matrices['Dxx'][task] = covariances_matrices_for_task['Dxx']	
		covariances_matrices['Dxy'][task] = covariances_matrices_for_task['Dxy']	
		covariances_matrices['Dyx'][task] = covariances_matrices_for_task['Dyx']	
		covariances_matrices['Dyy'][task] = covariances_matrices_for_task['Dyy']	
		covariances_matrices['Exx'][task] = covariances_matrices_for_task['Exx']	
		covariances_matrices['Exy'][task] = covariances_matrices_for_task['Exy']	
		covariances_matrices['Eyx'][task] = covariances_matrices_for_task['Eyx']	
		covariances_matrices['Eyy'][task] = covariances_matrices_for_task['Eyy']
		return covariances_matrices

	def compute_ep(self, pareto_set, obj_model_dict, minimize, con_models_dict, key, cand):
		if cand.ndim == 1:
                	cand = cand[None]
		...
		acq_dict, omegas = self.compute_and_evaluate_acquisition_function_all_factors(pareto_set, obj_model_dict, minimize, con_models_dict, \
					key, cand, self.options, acq, unconstrainedVariances, constrainedVariances)

		return acq_dict, omegas
	...
	def compute_and_evaluate_acquisition_function_all_factors(self, pareto_set, obj_model_dict, minimize, con_models_dict, key, cand, \
									opt, acq, unconstrainedVariances, constrainedVariances):
		omegas = dict()
               	for i in range(int(self.options['pesm_samples_per_hyper'])):
                	if pareto_set[ str(i) ] is None:
				continue;
                        else:
                        	#acq[ str(i) ] = ep(obj_model_dict, pareto_set[ str(i) ], minimize=minimize, \
                                 #               con_models=con_models_dict, input_space=self.input_space, Xtest)
				#TODO TODO TODO Validate this summation.
                        	acq, omegas[ str(i) ] = self.ppesmoc_computation_for_pareto_set_sample(obj_model_dict, pareto_set[ str(i) ], \
								minimize, con_models_dict, self.input_space, cand, \
                                                		unconstrainedVariances, constrainedVariances, acq, opt, key, i)
	...
	def compute_previous_operations_before_ep_algorithm(self, obj_model_dict, con_models_dict, pareto_set, cand):
			all_tasks = obj_model_dict.copy()
                        ...
                        mPred, Vpred, cholVpred, VpredInv, cholKstarstar, mPred_cons, Vpred_cons, cholVpred_cons, VpredInv_cons, cholKstarstar_cons = \
                                build_unconditioned_predictive_distributions(all_tasks, all_constraints, X)
                        jitter = assign_jitter(obj_model_dict, con_models_dict)
	...
	def ppesmoc_computation_for_pareto_set_sample(self, obj_model_dict, pareto_set, minimize, con_models_dict, input_space, cand, \
								unconstrainedVariances, constrainedVariances, acq, opt, key, ps_index):
		dict_par_points = None
		if key in self.reuse_EP_solution:
			dict_par_points = self.reuse_EP_solution[key]

		if key not in self.reuse_EP_solution or ps_index not in dict_par_points:
			a = self.compute_previous_operations_before_ep_algorithm(obj_model_dict, con_models_dict, pareto_set, cand)
		else:
			a = dict_par_points[ps_index]

                if opt['pesm_not_constrain_predictions'] == True:
			raise Exception("To have sense, parallel PESMOC needs to constraint predictions")
                else:
			predictionEP, omegas, a = self.ppesmoc_EP_computation(a, minimize, obj_model_dict, con_models_dict, key, ps_index)

		############################# TESTING #############################
		self.test_ppesmoc_EP_computation(cand, a, pareto_set, obj_model_dict, con_models_dict, key, ps_index)	
		###################################################################

                return self.compute_PPESMOC_approximation(predictionEP, obj_model_dict, con_models_dict, \
                                                unconstrainedVariances, constrainedVariances, acq), omegas

	##############################################################################################################
	#
	# FUNCTION DESCRIPTION
	# test_ppesmoc_EP_computation: Function that has to recieve the EP solution and the batch candidate X in
	#	order to test if the gradients are the same if the EP varies but are different if the X vary. What 
	#	self.ppesmoc_EP_computation outputs its the PPESMOC approximation, f(X), what self.compute_gradients_wrt_test_points
	#	returns are the gradients of f(X), i.e. \frac{\partial f(X)}{\partial X}. So, if we vary the EP solution, the gradients
	#	must not change. This is what it needs to be validated. So, we need to perform the next operation:
	#
	#        print "TEST A. Derivative. Changing X."
	#	 self.compute_gradients_wrt_test_points(X, ep)
        #        print "TEST. A. Test derivative. Changing X."
	#	 self.ppesmoc_EP_computation(X+epsilon, ep)-self.ppesmoc_EP_computation(X, ep)/epsilon 
	#        They must be the same, approximated by differences.
	#
	#        THIS TEST DOES NOT MAKE SENSE. IT IS CANCELLED.
	#	 TEST B. Derivative. Changing ep.
	#	 self.compute_gradients_wrt_test_points(X, ep)
        #        print "TEST. B. Test derivative. Changing ep."
	#	 self.ppesmoc_EP_computation(X, ep+epsilon)-self.ppesmoc_EP_computation(X, ep)/epsilon 
	#        They must be zero, approximated by differences.
	#
	# INPUTS
	# X are the batch candidates.
	# ep_solution is the solution of the ep_algorithm. A dictionary with keys predictionEP and omegas, what
	#   self.ppesmoc_EP_computation returns but in a dictionary.
	#
	#
	##############################################################################################################
	TEST_TASK = 'mocotoy_1'
	def test_ppesmoc_EP_computation(self, cand, ep_solution, pareto_set_point, obj_model_dict, con_model_dict, TEST_KEY, TEST_PS_INDEX):
		for TEST_POINT in range(cand.shape[0]):
			for TEST_DIMENSION in range(cand.shape[1]):
				cand_e = np.copy(cand)
				cand_e[TEST_POINT][TEST_DIMENSION] += PPESMOC.EPSILON
				minimize=True
				#X, necessary for tests....
				X, n_obs, n_pset, n_test, n_total = build_set_of_points_that_conditions_GPs(obj_model_dict, con_model_dict, \
		                        pareto_set_point, cand)
				acq, unconstrainedVariances, constrainedVariances = self.compute_unconstrained_variances_and_init_acq_fun \
                                                                                (obj_model_dict, cand, con_model_dict)
				mPred, Vpred, cholVpred, VpredInv, cholKstarstar, mPred_cons, Vpred_cons, cholVpred_cons, VpredInv_cons, cholKstarstar_cons = \
                		        build_unconditioned_predictive_distributions(obj_model_dict, con_model_dict, X)
				#X_e, necessary for tests...
				X_e, n_obs, n_pset, n_test, n_total = build_set_of_points_that_conditions_GPs(obj_model_dict, con_model_dict, \
                		        pareto_set_point, cand_e)
				acq_e, unconstrainedVariances_e, constrainedVariances_e = self.compute_unconstrained_variances_and_init_acq_fun \
                                                                                (obj_model_dict, cand_e, con_model_dict)
		                mPred_e, Vpred_e, cholVpred_e, VpredInv_e, cholKstarstar_e, mPred_cons_e, Vpred_cons_e, cholVpred_cons_e, VpredInv_cons_e, \
					cholKstarstar_cons_e = build_unconditioned_predictive_distributions(obj_model_dict, con_model_dict, X_e)
				jitter = assign_jitter(obj_model_dict, con_model_dict)
				ep_solution_e = create_data_structure_for_EP_computations_and_posterior_approximation(obj_model_dict, con_model_dict, n_obs, n_pset, n_test, n_total, len(obj_model_dict), len(con_model_dict), mPred_e, Vpred_e, VpredInv_e, cholKstarstar_e, mPred_cons_e, Vpred_cons_e, VpredInv_cons_e, cholKstarstar_cons_e, jitter, X_e)
				########################################################################################################

				#f(X,ep)
				Omegas = dict()
				predictionEP, Omegas_p, a = self.ppesmoc_EP_computation(ep_solution, minimize, obj_model_dict, con_model_dict, TEST_KEY, TEST_PS_INDEX)
				Omegas['0'] = Omegas_p
				acq_dict, constrainedVariances =  self.compute_PPESMOC_approximation(predictionEP, obj_model_dict, con_model_dict, \
				...
				#f(X+e,ep)
				Omegas_e = dict()
				predictionEP_X_e, Omegas_X_e, a_e = self.ppesmoc_EP_computation(ep_solution_e, minimize, obj_model_dict, con_model_dict, TEST_KEY, TEST_PS_INDEX)
				Omegas_e['0'] = Omegas_X_e
				acq_dict_X_e, constrainedVariances_e =  self.compute_PPESMOC_approximation(predictionEP_X_e, obj_model_dict, con_model_dict, \
                                                unconstrainedVariances_e, constrainedVariances_e, acq_e)
				total_acq_X_e = 0.0
		                for task in tasks:
		                        total_acq_X_e += acq_dict_X_e[ task ]
				#f(X,ep_e)
				#predictionEP_ep_e, Omegas_ep_e = self.ppesmoc_EP_computation(ep_solution_e, minimize, all_tasks, all_constraints, PPESMOC.TEST_KEY, PPESMOC.TEST_PS_INDEX)
				#acq_dict_ep_e =  self.compute_PPESMOC_approximation(predictionEP_ep_e, all_tasks, all_constraints, \
                                                #unconstrainedVariances, constrainedVariances, acq)
				#import pdb; pdb.set_trace();
				total_acq_grad, acq_grad, unconstrained_variances_derivative, constrained_variances_derivative, \
					covariances_matrices_gradients = self.compute_gradients_wrt_test_points(obj_model_dict, con_model_dict, self.sampled_pareto_set, tasks, cand, Omegas)

				#Tests failed, possible explanations. TODO, research about this.
				#CURRENT STATE: Matrices B and C validated and OK. Matrices D, E error. Final expression error.
				#I suspect that there might be two sources of error here, that might explain the failure of the tests.
				# KO a) In self.compute_PPESMOC_approximation: value = 0.5 * np.log(np.linalg.det(unconstrainedVariances[t])) 
				# - 0.5 * np.log(np.linalg.det(constrainedVariances[t])). Where are the unconstrainedVariances in 
				# self.compute_gradients_wrt_test_points? They are not computed nor substracted, only unconstrainedVariances[t].
				# b) Bad Omegas management. It would be strange, as they are passed. But it is a possibility. As the indexes are
				# somewhat strange.
				# First ridiculous thing, indexing Omegas by n_task instead of task, change this shit now.
				# SECOND RIDICULOUS THING: OMEGAS. OMEGAS ARE JUST PREDICTED MINUS ORIGINAL. WHAT STUPIDITY, IF YOU GO
				# SUMMING THE SOURCES OF ERROR ARE ENORUMOUS, I THINK THAT OMEGAS SHOULD BE EXTRACTED FROM a A POSTERIORI, NOT
				# BEING COMPUTED!!!!!!!!!.
				# OBJECTIVES: OMEGAS ARE : a['Vinv'] - A['VpredInv'] (AND ITS TASK)
				# CONSTRAINTS: OMEGAS ARE : a['Vinv_cons'] - A['VpredInv_cons']
				# AT THE END OF THE EP ALGORITHM, ALL THE FACTORS WILL BE CONTAINED IN THOSE MATRICES.
				# AND XX,XY,YX,YY ARE EXTRACTED FROM THERE INDEXING. LETS GO!!!!!!!!!!!!!!!!!!!
				# KO c) Modify iteration data. This thing is kind of an add-on and it is strange. Might also be the thing.
				# I deleted this thing, it was wrong and maybe it did not add anything.
				#It must hold that (f(X+e)-f(X))/e = dev(f(X)).
				obj_keys = obj_model_dict.keys()
				con_keys = con_model_dict.keys()
				print "Starting PPESMOC TESTS"
				print "======================"
				print "Point + " + str(TEST_POINT) + ". Dimension = " + str(TEST_DIMENSION)
				print "TEST 1. Total Derivative. Changing X."
				print total_acq_grad[TEST_POINT][TEST_DIMENSION]
				print "TEST 1. Total Test derivative. Changing X."
				print (total_acq_X_e - total_acq) / PPESMOC.EPSILON
				print "TEST 2. " + obj_keys[0] + " Derivative. Changing X"
				print acq_grad[0][TEST_POINT][TEST_DIMENSION]
				print "TEST 2. " + obj_keys[0] + " Test Derivative. Changing X"
				print (acq_dict_X_e[obj_keys[0]] - acq_dict[obj_keys[0]]) / PPESMOC.EPSILON
				print "TEST 3. " + obj_keys[1] + " Derivative. Changing X"
                                print acq_grad[1][TEST_POINT][TEST_DIMENSION]
                                print "TEST 3. " + obj_keys[1] + " Test Derivative. Changing X"
                                print (acq_dict_X_e[obj_keys[1]] - acq_dict[obj_keys[1]]) / PPESMOC.EPSILON
				print "TEST 4. " + con_keys[0] + " Derivative. Changing X"
                                print acq_grad[2][TEST_POINT][TEST_DIMENSION]
                                print "TEST 4. " + con_keys[0] + " Test Derivative. Changing X"
                                print (acq_dict_X_e[con_keys[0]] - acq_dict[con_keys[0]]) / PPESMOC.EPSILON
				print "TEST 5. " + con_keys[1] + " Derivative. Changing X"
                                print acq_grad[3][TEST_POINT][TEST_DIMENSION]
                                print "TEST 5. " + con_keys[1] + " Test Derivative. Changing X"
                                print (acq_dict_X_e[con_keys[1]] - acq_dict[con_keys[1]]) / PPESMOC.EPSILON
				print "TEST 6. " + obj_keys[0] + " unconstrained covariance matrix. Derivative. Changing X"
				print unconstrained_variances_derivative[obj_keys[0]][str(TEST_POINT)][str(TEST_DIMENSION)]
				print "TEST 6. " + obj_keys[0] + " unconstrained covariance matrix. Test Derivative. Changing X"
				print (unconstrainedVariances_e[obj_keys[0]] - unconstrainedVariances[obj_keys[0]]) / PPESMOC.EPSILON
				print "TEST 7. " + obj_keys[1] + " unconstrained covariance matrix. Derivative. Changing X"
                                print unconstrained_variances_derivative[obj_keys[1]][str(TEST_POINT)][str(TEST_DIMENSION)]
                                print "TEST 7. " + obj_keys[1] + " unconstrained covariance matrix. Test Derivative. Changing X"
                                print (unconstrainedVariances_e[obj_keys[1]] - unconstrainedVariances[obj_keys[1]]) / PPESMOC.EPSILON
				print "TEST 8. " + con_keys[0] + " unconstrained covariance matrix. Derivative. Changing X"
                                print unconstrained_variances_derivative[con_keys[0]][str(TEST_POINT)][str(TEST_DIMENSION)]
                                print "TEST 8. " + con_keys[0] + " unconstrained covariance matrix. Test Derivative. Changing X"
                                print (unconstrainedVariances_e[con_keys[0]] - unconstrainedVariances[con_keys[0]]) / PPESMOC.EPSILON
				print "TEST 9. " + con_keys[1] + " unconstrained covariance matrix. Derivative. Changing X"
                                print unconstrained_variances_derivative[con_keys[1]][str(TEST_POINT)][str(TEST_DIMENSION)]
                                print "TEST 9. " + con_keys[1] + " unconstrained covariance matrix. Test Derivative. Changing X"
                                print (unconstrainedVariances_e[con_keys[1]] - unconstrainedVariances[con_keys[1]]) / PPESMOC.EPSILON
				print "TEST 10. " + obj_keys[0] + ". Testing inverse of B, C."
				print "In PPESMOC, represented by VpredInv, two alternatives, from now on 1: "
				print chol2inv(spla.cholesky(obj_model_dict['mocotoy_1'].predict(X,full_cov=True)[1]))[n_pset+n_obs:n_total,n_pset+n_obs:n_total]
				print VpredInv['mocotoy_1'][n_pset+n_obs:n_total,n_pset+n_obs:n_total]
				print "In PPESMOC Gradients, matrix C of the testing points:"
				print covariances_matrices_gradients['Cyy']['mocotoy_1']


				#Constrained tests. (E)
				print "TEST 10. " + obj_keys[0] + " constrained covariance matrix. Derivative. Changing X"
                                print constrained_variances_derivative[obj_keys[0]][str(TEST_POINT)][str(TEST_DIMENSION)]
                                print "TEST 10. " + obj_keys[0] + " constrained covariance matrix. Test Derivative. Changing X"
                                print (constrainedVariances_e[obj_keys[0]] - constrainedVariances[obj_keys[0]]) / PPESMOC.EPSILON
                                print "TEST 11. " + obj_keys[1] + " constrained covariance matrix. Derivative. Changing X"
                                print constrained_variances_derivative[obj_keys[1]][str(TEST_POINT)][str(TEST_DIMENSION)]
                                print "TEST 11. " + obj_keys[1] + " constrained covariance matrix. Test Derivative. Changing X"
                                print (constrainedVariances_e[obj_keys[1]] - constrainedVariances[obj_keys[1]]) / PPESMOC.EPSILON
                                print "TEST 12. " + con_keys[0] + " constrained covariance matrix. Derivative. Changing X"
                                print constrained_variances_derivative[con_keys[0]][str(TEST_POINT)][str(TEST_DIMENSION)]
                                print "TEST 12. " + con_keys[0] + " constrained covariance matrix. Test Derivative. Changing X"
                                print (constrainedVariances_e[con_keys[0]] - constrainedVariances[con_keys[0]]) / PPESMOC.EPSILON
                                print "TEST 13. " + con_keys[1] + " constrained covariance matrix. Derivative. Changing X"
                                print constrained_variances_derivative[con_keys[1]][str(TEST_POINT)][str(TEST_DIMENSION)]
                                print "TEST 13. " + con_keys[1] + " constrained covariance matrix. Test Derivative. Changing X"
                                print (constrainedVariances_e[con_keys[1]] - constrainedVariances[con_keys[1]]) / PPESMOC.EPSILON


				print "TEST 14. " + obj_keys[0] + ". Testing inverse of B, C."
                                print "In PPESMOC, represented by VpredInv, two alternatives, from now on 1: "
                                print chol2inv(spla.cholesky(obj_model_dict['mocotoy_1'].predict(X,full_cov=True)[1]))[n_pset+n_obs:n_total,n_pset+n_obs:n_total]
                                print VpredInv['mocotoy_1'][n_pset+n_obs:n_total,n_pset+n_obs:n_total]
                                print "In PPESMOC Gradients, matrix C of the testing points:"
                                print covariances_matrices_gradients['Cyy']['mocotoy_1']
				print "TEST 15. " + obj_keys[1] + ". Testing inverse of B, C."
                                print "In PPESMOC, represented by VpredInv, two alternatives, from now on 1: "
                                print VpredInv['mocotoy_2'][n_pset+n_obs:n_total,n_pset+n_obs:n_total]
                                print "In PPESMOC Gradients, matrix C of the testing points:"
                                print covariances_matrices_gradients['Cyy']['mocotoy_2']
				print "TEST 16. " + con_keys[0] + ". Testing inverse of B, C."
                                print "In PPESMOC, represented by VpredInv, two alternatives, from now on 1: "
                                print VpredInv_cons[con_keys[0]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]
                                print "In PPESMOC Gradients, matrix C of the testing points:"
                                print covariances_matrices_gradients['Cyy'][con_keys[0]]
				print "TEST 17. " + con_keys[1] + ". Testing inverse of B, C."
                                print "In PPESMOC, represented by VpredInv, two alternatives, from now on 1: "
                                print VpredInv_cons[con_keys[1]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]
                                print "In PPESMOC Gradients, matrix C of the testing points:"
                                print covariances_matrices_gradients['Cyy'][con_keys[1]]

				#Constrained tests. (D)
				print "TEST 18. " + obj_keys[0] + ". Testing D_yy."
                                print "In PPESMOC, represented by VInv:"
                                print a['Vinv'][obj_keys[0]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]
                                print "In PPESMOC Gradients, matrix Dyy of the testing points:"
                                print covariances_matrices_gradients['Dyy'][obj_keys[0]]
				print "TEST 19. " + obj_keys[1] + ". Testing D_yy."
                                print "In PPESMOC, represented by VInv:"
                                print a['Vinv'][obj_keys[1]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]
                                print "In PPESMOC Gradients, matrix Dyy of the testing points:"
                                print covariances_matrices_gradients['Dyy'][obj_keys[1]]
				print "TEST 20. " + con_keys[0] + ". Testing D_yy."
                                print "In PPESMOC, represented by VInv_cons:"
                                print a['Vinv_cons'][con_keys[0]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]
                                print "In PPESMOC Gradients, matrix Dyy of the testing points:"
                                print covariances_matrices_gradients['Dyy'][con_keys[0]]
				print "TEST 21. " + con_keys[1] + ". Testing D_yy."
                                print "In PPESMOC, represented by VInv_cons:"
                                print a['Vinv_cons'][con_keys[1]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]
                                print "In PPESMOC Gradients, matrix Dyy of the testing points:"
                                print covariances_matrices_gradients['Dyy'][con_keys[1]]
				print "TEST 22. " + obj_keys[0] + ". Testing D_xx."
                                print "In PPESMOC, represented by VInv:"
                                print a['Vinv'][obj_keys[0]][n_obs:n_obs+n_pset,n_obs:n_obs+n_pset]
                                print "In PPESMOC Gradients, matrix Dxx of the testing points:"
                                print covariances_matrices_gradients['Dxx'][obj_keys[0]]
				print "TEST 23. " + obj_keys[1] + ". Testing D_xx."
                                print "In PPESMOC, represented by VInv:"
                                print a['Vinv'][obj_keys[1]][n_obs:n_obs+n_pset,n_obs:n_obs+n_pset]
                                print "In PPESMOC Gradients, matrix Dxx of the testing points:"
                                print covariances_matrices_gradients['Dxx'][obj_keys[1]]
				print "TEST 24. " + con_keys[0] + ". Testing D_xx."
                                print "In PPESMOC, represented by VInv_cons:"
                                print a['Vinv_cons'][con_keys[0]][n_obs:n_obs+n_pset,n_obs:n_obs+n_pset]
                                print "In PPESMOC Gradients, matrix Dxx of the testing points:"
                                print covariances_matrices_gradients['Dxx'][con_keys[0]]
				print "TEST 25. " + con_keys[1] + ". Testing D_xx."
                                print "In PPESMOC, represented by VInv:"
                                print a['Vinv_cons'][con_keys[1]][n_obs:n_obs+n_pset,n_obs:n_obs+n_pset]
                                print "In PPESMOC Gradients, matrix Dxx of the testing points:"
                                print covariances_matrices_gradients['Dxx'][con_keys[1]]
				print "TEST 26. " + obj_keys[0] + ". Testing D_xy."
                                print "In PPESMOC, represented by VInv:"
                                print a['Vinv'][obj_keys[0]][n_obs:n_obs+n_pset,n_obs+n_pset:n_total]
                                print "In PPESMOC Gradients, matrix Dxy of the testing points:"
                                print covariances_matrices_gradients['Dxy'][obj_keys[0]]
				print "TEST 27. " + obj_keys[1] + ". Testing D_xy."
                                print "In PPESMOC, represented by VInv:"
                                print a['Vinv'][obj_keys[1]][n_obs:n_obs+n_pset,n_obs+n_pset:n_total]
                                print "In PPESMOC Gradients, matrix Dxy of the testing points:"
                                print covariances_matrices_gradients['Dxy'][obj_keys[1]]
				print "TEST 28. " + con_keys[0] + ". Testing D_xy."
                                print "In PPESMOC, represented by VInv_cons:"
                                print a['Vinv_cons'][con_keys[0]][n_obs:n_obs+n_pset,n_obs+n_pset:n_total]
                                print "In PPESMOC Gradients, matrix Dxy of the testing points:"
                                print covariances_matrices_gradients['Dxy'][con_keys[0]]
				print "TEST 29. " + con_keys[1] + ". Testing D_xy."
                                print "In PPESMOC, represented by VInv_cons:"
                                print a['Vinv_cons'][con_keys[1]][n_obs:n_obs+n_pset,n_obs+n_pset:n_total]
                                print "In PPESMOC Gradients, matrix Dxy of the testing points:"
                                print covariances_matrices_gradients['Dxy'][con_keys[1]]

				#Constrained tests. (E)
                                print "TEST 30. " + obj_keys[0] + ". Testing E_yy."
                                print "In PPESMOC, represented by V:"
                                print a['V'][obj_keys[0]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]
                                print "In PPESMOC Gradients, matrix Eyy of the testing points:"
                                print covariances_matrices_gradients['Eyy'][obj_keys[0]]
                                print "TEST 31. " + obj_keys[1] + ". Testing E_yy."
                                print "In PPESMOC, represented by V:"
                                print a['V'][obj_keys[1]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]
                                print "In PPESMOC Gradients, matrix Eyy of the testing points:"
                                print covariances_matrices_gradients['Eyy'][obj_keys[1]]
                                print "TEST 32. " + con_keys[0] + ". Testing E_yy."
                                print "In PPESMOC, represented by V_cons:"
                                print a['V_cons'][con_keys[0]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]
                                print "In PPESMOC Gradients, matrix Eyy of the testing points:"
                                print covariances_matrices_gradients['Eyy'][con_keys[0]]
                                print "TEST 33. " + con_keys[1] + ". Testing E_yy."
                                print "In PPESMOC, represented by V_cons:"
                                print a['V_cons'][con_keys[1]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]
                                print "In PPESMOC Gradients, matrix Eyy of the testing points:"
                                print covariances_matrices_gradients['Eyy'][con_keys[1]]

				#TEST DERIVATIVES. D.
				print "TEST 34. " + obj_keys[0] + ". Testing D with derivatives. Derivative"
				print self.dev_Dyy_xij(self.cov_and_blocks_task(covariances_matrices_gradients,obj_keys[0]), obj_model_dict[obj_keys[0]], TEST_POINT, TEST_DIMENSION)
				print "PPESMOC Test derivative value. Epsilon."
				print (a_e['Vinv'][obj_keys[0]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]-a['Vinv'][obj_keys[0]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]) / PPESMOC.EPSILON
				print "TEST 35. " + obj_keys[1] + ". Testing D with derivatives. Derivative"
                                print self.dev_Dyy_xij(self.cov_and_blocks_task(covariances_matrices_gradients,obj_keys[1]), obj_model_dict[obj_keys[1]], TEST_POINT, TEST_DIMENSION)
                                print "PPESMOC Test derivative value. Epsilon."
                                print (a_e['Vinv'][obj_keys[1]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]-a['Vinv'][obj_keys[1]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]) / PPESMOC.EPSILON
				print "TEST 36. " + con_keys[0] + ". Testing D with derivatives. Derivative"
                                print self.dev_Dyy_xij(self.cov_and_blocks_task(covariances_matrices_gradients,con_keys[0]), con_model_dict[con_keys[0]], TEST_POINT, TEST_DIMENSION)
                                print "PPESMOC Test derivative value. Epsilon."
                                print (a_e['Vinv_cons'][con_keys[0]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]-a['Vinv_cons'][con_keys[0]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]) / PPESMOC.EPSILON
				print "TEST 37. " + con_keys[1] + ". Testing D with derivatives. Derivative"
                                print self.dev_Dyy_xij(self.cov_and_blocks_task(covariances_matrices_gradients,con_keys[1]), con_model_dict[con_keys[1]], TEST_POINT, TEST_DIMENSION)
                                print "PPESMOC Test derivative value. Epsilon."
                                print (a_e['Vinv_cons'][con_keys[1]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]-a['Vinv_cons'][con_keys[1]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]) / PPESMOC.EPSILON

				#If the EP factors are zero, print self.dev_Dyy_xij(self.cov_and_blocks_task(covariances_matrices_gradients,con_keys[1]), con_model_dict[con_keys[1]], TEST_POINT, TEST_DIMENSION) is equal to (a_e['VpredInv_cons'][con_keys[1]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]-a['VpredInv_cons'][con_keys[1]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]) / PPESMOC.EPSILON, which is OK, but if the EP factors are not zero: print self.dev_Dyy_xij(self.cov_and_blocks_task(covariances_matrices_gradients,con_keys[1]), con_model_dict[con_keys[1]], TEST_POINT, TEST_DIMENSION) is not equal to (a_e['Vinv_cons'][con_keys[1]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]-a['Vinv_cons'][con_keys[1]][n_pset+n_obs:n_total,n_pset+n_obs:n_total]) / PPESMOC.EPSILON. Having that the gradients method is 100% tested and works OK and that the PPESMOC block matrices B,C,D and E are the same that the matrices of the gradients method the ONLY thing that can happen is that the EP factors are badly assigned to matrices C. Why? I think that block Bxx should have shape par_set+obs
				#It must hold that (f(X,EP+e)-f(X,EP))/e = 0 != dev(f(X)).
				#print "TEST B. Derivative. Changing ep."
				#print acq_grad[PPESMOC.TEST_TASK]
				#print "TEST. B. Test derivative. Changing ep."
				#print (acq_dict_ep_e[PPESMOC.TEST_TASK] - acq_dict[PPESMOC.TEST_TASK]) / PPESMOC.EPSILON
		print "============================="
		return 

	#DANIEL doubt: Override some information of previous a for new EP computation?
	def modify_iteration_data(self, a, X, mPred, Vpred, cholVpred, VpredInv, cholKstarstar, mPred_cons, Vpred_cons, cholVpred_cons, VpredInv_cons, cholKstarstar_cons):
		#Ask Daniel to confirm this.
		a['X'] = X
		a['mPred'] = mPred
		a['Vpred'] = Vpred
		a['cholVpred'] = cholVpred
		a['VpredInv'] = VpredInv
		a['cholKstarstar'] = cholKstarstar
		a['mPred_cons'] = mPred_cons
		a['Vpred_cons'] = Vpred_cons
		a['cholVpred_cons'] = cholVpred_cons
		a['VpredInv_cons'] = VpredInv_cons
		a['cholKstarstar_cons'] = cholKstarstar_cons
		return a
	...
	def ppesmoc_EP_computation(self, a, minimize, all_tasks, all_constraints, key, ps_index):
		convergence = False
    		damping     = 0.5
    		iteration   = 1
		...
		self.reuse_EP_solution[key][ps_index] = a
	...
	def get_test_predictive_distributions(self, a):
		...
		omegas = {
			'omega_xx' : defaultdict(lambda: np.zeros((n_obs + n_pset, n_obs + n_pset))),
                        'omega_xy' : defaultdict(lambda: np.zeros((n_obs + n_pset, n_test))),
                        'omega_yx' : defaultdict(lambda: np.zeros((n_test, n_obs + n_pset))),
                        'omega_yy' : defaultdict(lambda: np.zeros((n_test, n_test))),
		}
		for obj in a['objs'].keys():
			...
			omegas['omega_xx'][obj] = a['Vinv'][obj][0 : n_obs + n_pset, 0 : n_obs + n_pset] - a['VpredInv'][obj][0 : n_obs + n_pset, 0 : n_obs + n_pset]
                	omegas['omega_xy'][obj] = a['Vinv'][obj][0 : n_obs + n_pset, n_obs + n_pset : n_total] - a['VpredInv'][obj][0 : n_obs + n_pset, n_obs + n_pset : n_total]
                	omegas['omega_yx'][obj] = a['Vinv'][obj][n_obs + n_pset : n_total, 0 : n_obs + n_pset] - a['VpredInv'][obj][n_obs + n_pset : n_total, 0 : n_obs + n_pset]
                	omegas['omega_yy'][obj] = a['Vinv'][obj][n_obs + n_pset : n_total, n_obs + n_pset : n_total] - a['VpredInv'][obj][n_obs + n_pset : n_total, n_obs + n_pset : n_total]
		for cons in a['cons'].keys():
			...
			omegas['omega_xx'][cons] = a['Vinv_cons'][cons][0 : n_obs + n_pset, 0 : n_obs + n_pset] - a['VpredInv_cons'][cons][0 : n_obs + n_pset, 0 : n_obs + n_pset]
                        omegas['omega_xy'][cons] = a['Vinv_cons'][cons][0 : n_obs + n_pset, n_obs + n_pset : n_total] - a['VpredInv_cons'][cons][0 : n_obs + n_pset, n_obs + n_pset : n_total]
                        omegas['omega_yx'][cons] = a['Vinv_cons'][cons][n_obs + n_pset : n_total, 0 : n_obs + n_pset] - a['VpredInv_cons'][cons][n_obs + n_pset : n_total, 0 : n_obs + n_pset]
                        omegas['omega_yy'][cons] = a['Vinv_cons'][cons][n_obs + n_pset : n_total, n_obs + n_pset : n_total] - a['VpredInv_cons'][cons][n_obs + n_pset : n_total, n_obs + n_pset : n_total]
		
		return predictive_distributions, omegas, a
		...
		for cons in all_constraints:
				...
				np.sum(a['g_c_hfhat'][ :, : , ntask ], axis = 1))
				 ...
				 np.sum(a['h_c_hfhat'][ :, : , ntask ], axis = 1))
                	...
                	a['m_nat_cons'][cons] = np.dot(a['VpredInv_cons'][cons], a['mPred_cons'][cons]) + mTilde_cons
	                a['m_cons'][cons] = np.dot(a['V_cons'][cons], a['m_nat_cons'][ cons ])
			#vTilde_cons_back[cons] = vTilde_cons
	                ntask = ntask + 1
		ntask = 0
	        for obj in all_tasks:
			...
			vTilde[ n_obs + n_pset : n_total, n_obs : n_obs + n_pset ] = a['ghfhat'][ :, :, ntask, 0, 1]
				...
				np.sum(a['hhfhat'][ :, : , ntask, 0 ], axis = 1))
        	        ...
        	        a['m'][obj] = np.dot(a['V'][obj], a['m_nat'][ obj ])
                	ntask = ntask + 1
			#vTilde_back[obj] = vTilde

		return a

	def update_full_factors(self, a, damping, minimize=True, no_negative_variances_nor_nands = False):
		# used to switch between minimizing and maximizing
		sgn = -1.0 if minimize else 1.0
		...
		# First we update the external (easy) factors that only affect the constraints
    		...
    		for cons in all_constraints:
        		#c_m_external[ n_task, : ] = a['m_cons'][ cons ][ 0 : n_pset ]
        		c_m_external[ n_task, : ] = a['m_cons'][ cons ][ n_obs : n_total ]
        		#c_v_external[ n_task, : ] = np.diag(a['V_cons'][ cons ])[ 0 : n_pset ]
        		c_v_external[ n_task, : ] = np.diag(a['V_cons'][ cons ])[ n_obs : n_total ]
        		n_task += 1	
		...
		n_task = 0
    		for obj in all_tasks:
        		...
        		m_obs[ n_task, :, : ] = np.tile(a['m'][ obj ][ 0 : n_obs ], n_pset).reshape((n_pset, n_obs))
        		#m_pset[ n_task, :, : ] = np.tile(a['m'][ obj ][ 0 : n_pset ], n_obs).reshape((n_obs, n_pset)).T
        		m_pset[ n_task, :, : ] = np.tile(a['m'][ obj ][ n_obs : n_total ], n_obs).reshape((n_obs, n_pset)).T
        		#v_cov[ n_task, :, : ] = a['V'][ obj ][ 0 : n_pset, n_pset : n_total ]
        		v_cov[ n_task, :, : ] = a['V'][ obj ][ n_obs : n_total, 0 : n_obs ]
        		#v_obs[ n_task, :, : ] = np.tile(np.diag(a['V'][ obj ])[ n_pset : n_total ], n_pset).reshape((n_pset, n_obs))
        		v_obs[ n_task, :, : ] = np.tile(np.diag(a['V'][ obj ])[ 0 : n_obs ], n_pset).reshape((n_pset, n_obs))
        		#v_pset[ n_task, :, : ] = np.tile(np.diag(a['V'][ obj ])[ 0 : n_pset ], n_obs).reshape((n_obs, n_pset)).T
        		v_pset[ n_task, :, : ] = np.tile(np.diag(a['V'][ obj ])[ n_obs : n_total ], n_obs).reshape((n_obs, n_pset)).T
        		n_task += 1
		...
		for obj in all_tasks:
        		#m_pset1[ n_task, :, : ] = np.tile(a['m'][ obj ][ 0 : n_pset ], n_pset).reshape((n_pset, n_pset))
        		m_pset1[ n_task, :, : ] = np.tile(a['m'][ obj ][ n_obs : n_total ], n_pset).reshape((n_pset, n_pset))
        		#m_pset2[ n_task, :, : ] = np.tile(a['m'][ obj ][ 0 : n_pset ], n_pset).reshape((n_pset, n_pset)).T
        		m_pset2[ n_task, :, : ] = np.tile(a['m'][ obj ][ n_obs : n_total ], n_pset).reshape((n_pset, n_pset)).T
        		#v_cov[ n_task, :, : ] = a['V'][ obj ][ 0 : n_pset, 0 : n_pset ]
        		v_cov[ n_task, :, : ] = a['V'][ obj ][ n_obs : n_total, n_obs : n_total ]
        		v_cov[ n_task, :, : ] = v_cov[ n_task, :, : ] - np.diag(np.diag(v_cov[ n_task, :, : ]))
        		#v_pset1[ n_task, :, : ] = np.tile(np.diag(a['V'][ obj ])[ 0 : n_pset ], n_pset).reshape((n_pset, n_pset))
        		v_pset1[ n_task, :, : ] = np.tile(np.diag(a['V'][ obj ])[ n_obs : n_total ], n_pset).reshape((n_pset, n_pset))
        		#v_pset2[ n_task, :, : ] = np.tile(np.diag(a['V'][ obj ])[ 0 : n_pset ], n_pset).reshape((n_pset, n_pset)).T
        		v_pset2[ n_task, :, : ] = np.tile(np.diag(a['V'][ obj ])[ n_obs : n_total ], n_pset).reshape((n_pset, n_pset)).T
        		n_task += 1

    		n_task = 0
    		for cons in all_constraints:
        		#c_m_pset[ n_task, :, : ] = a['m_cons'][ cons ][ 0 : n_pset ]
        		c_m_pset[ n_task, :, : ] = a['m_cons'][ cons ][ n_obs : n_total ]
        		#c_v_pset[ n_task, :, : ] = np.diag(a['V_cons'][ cons ])[0 : n_pset]
        		c_v_pset[ n_task, :, : ] = np.diag(a['V_cons'][ cons ])[n_obs : n_total]
        		n_task += 1
		...
		#Here it will go the code of predictEP factors, it is easy at the end! :D Other option is that it may go 
		#after updating marginals.
    		return a
	...
	def compute_full_updates_and_reduce_damping_if_fail(self, a, damping, minimize, aOld):
		update_correct = False
	        ...
	        while update_correct == False:
	            error = False
	            try:
	                aNew = update_full_Factors(copy.deepcopy(a), damping_inner, minimize = minimize,
	                         no_negative_variances_nor_nands = False)
        	        aNew = self.update_full_marginals(copy.deepcopy(aNew))
			#aNew = compute_choleskys(copy.deepcopy(aNew), aNew['objs'], aNew['cons'])	UNUSEFUL NOW.
			#Here, it may go the code of predictEP, I think so, it would fit and be in the same convergence loop, but
			#I am not sure. What it is sure is that the convergence loop would have to dissapear.
			#The logic of this method must be put in updateFactors_fast_daniel_no_constraints_pareto_set_robust.
			#aNew = predictEP_multiple_iter_optim_robust(aNew['objs'], aNew['cons'], copy.deepcopy(aNew), True, minimize, damping_inner)
	            except npla.linalg.LinAlgError as e:
	                error = True
	            if error == False:
	                if fail == True and second_update == False:
	                        ...
	                        update_correct = True
				#self.update_omegas(aNew, vTilde, vTilde_cons)
        	    else:
	                a = copy.deepcopy(aOld)
        	        damping_inner = damping_inner * 0.5
	                fail = True
	...
	def compute_PPESMOC_approximation(self, predictionEP, obj_models_dict, con_models, unconstrainedVariances, constrainedVariances, acq):
                ...
                for obj in obj_models_dict:
		    np.fill_diagonal(predictionEP_obj[ obj ], np.diagonal(predictionEP_obj[ obj ]) + obj_models_dict[ obj ].noise_value())
                    constrainedVariances[ obj ] = predictionEP_obj[ obj ]

                for cons in con_models:
		    np.fill_diagonal(predictionEP_cons[ cons ], np.diagonal(predictionEP_cons[ cons ]) + con_models[ cons ].noise_value())
		...
		N = unconstrainedVariances[cons].shape[0] # Used for det(cA) = np.power(c,n)det(A) : n = shape(A). The matrix cookbook.
                for t in unconstrainedVariances:
                    #value = 0.5 * np.log(2 * np.pi * np.e * unconstrainedVariances[t]) - \
                    #    0.5 * np.log(2 * np.pi * np.e * constrainedVariances[t])
			
		    #value =  0.5 * np.log(np.power(2.0 * np.pi * np.e, N)) + \ TEST THIS TO BE SURE, BUT APPARENTLY, IT IS SURE, AS IT IS ONLY A SCALAR.
		    value = 0.5 * np.log(np.linalg.det(unconstrainedVariances[t])) - 0.5 * np.log(np.linalg.det(constrainedVariances[t]))
		    ...
		    value = np.max(value, 0)
                    ...
                    acq[t] += value
                return acq, constrainedVariances
