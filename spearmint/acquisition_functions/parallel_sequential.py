...
class parallel_sequential(AbstractAcquisitionFunction):
	...
	def __init__(self, num_dims, verbose=True, input_space=None, grid=None, opt = None):
                ...
                self.cached_computed_points = None
                ...
                self.options["parallel_sequential_evaluations"] = self.options['batch_size']
                ...
                self.options["initial_point"] = "ON"
                self.chooser = chooser_module.init(input_space, self.options)
		return

        def compute_optimum_points(self, obj_model_dict, con_models_dict, tasks, tasks_values):
                models = dict(obj_model_dict)
                models.update(con_models_dict)
                model_values = models.values()
                self.chooser.models = models.copy()

                #Building hypers.
                self.hypers = dict.fromkeys(tasks)
                for hyper in self.hypers:
                    self.hypers[hyper] = dict()
                    self.hypers[hyper]['chain length'] = 21
                    self.hypers[hyper]['hypers'] = dict()
                    if tasks_values[hyper].type == u'constraint':
                        self.hypers[hyper]['hypers'] = con_models_dict[hyper]._hypers_list[0]
                    else:
                        self.hypers[hyper]['hypers'] = obj_model_dict[hyper]._hypers_list[0]

                x_opt = np.array([])

                for i in range(self.options["parallel_sequential_evaluations"]):

                    for task in tasks_values:
                        tasks_values[task].pending = x_opt

                    if i == 0: #Nothing to pend, necessary to generate grid for suggest.
                        self.chooser.set_black_boxes(tasks_values.copy())
                        self.chooser.generate_grid(tasks_values.copy())
                    else: #Pend new observations to the GP and condition the predictive mean.
                        self.chooser.fit(tasks_values, self.hypers)

                    if x_opt.shape[0] == 0:
                        if self.num_dims > 1:
                            x_opt = np.array([self.chooser.suggest(tasks)[0]])
                            self.options["initial_point"] = "OFF"
                        else:
                            x_opt = np.array([self.chooser.suggest(tasks)[0]])[0]
                    else:
                        x_opt = np.vstack((x_opt, self.chooser.suggest(tasks)[0]))

                self.cached_computed_points = self.input_space.to_unit(x_opt)

                for task in tasks_values:
                    tasks_values[task].pending = np.array([])

                return self.cached_computed_points
	...
	def acquisition(self, obj_model_dict, con_models_dict, cand, current_best, compute_grad, minimize=True, tasks=None, tasks_values=None):
            if cand.shape[0] > 2:
                return np.array([-np.sum(np.power(point-self.cached_computed_points, 2.0)) for point in cand])
            else:
                cand = cand.reshape(self.cached_computed_points.shape)
    
                distance = - np.sum(np.power(cand - self.cached_computed_points, 2.0))
                if not compute_grad:
                    return distance
                else:
                    grads = - 2.0 * (cand - self.cached_computed_points)
