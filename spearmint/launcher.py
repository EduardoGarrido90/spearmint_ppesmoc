...
def launch(db_address, experiment_name, job_id):
	...
	#In parallel BO, we return a list of dictionaries.
	if isinstance(result, list):
		if not isinstance(result[0], dict):
			if len(job['tasks']) == 1: #Only supports one objective from now, needs to be expanded to multiple objectives.
				result = {job['tasks'][0] : result}
		else: #Not tested yet.
			if "objective" in result[0] and "constraints" in result[0]: #Assuming that if it appears in the first, in the rest too.
				result_new = dict()
				for point_id in range(len(result)): #Number of points of the Parallel BO batch.
					result_new['%s%d' % (DEFAULT_TASK_NAME, point_id)] = result[point_id]["objective"]
					for i in xrange(len(result[0]["constraints"])): #Assuming all points have the same constraints.
			                    result_new['%s%d-%d' % (DEFAULT_CONSTRAINT_NAME, i, point_id)] = result["constraints"][point_id][i]
                			result = result_new
...
#        if set(result.keys()) != set(job['tasks']):
	...
	#            if set(result.keys()).union(['NaN']) != set(job['tasks']):
		...
		for res in result:
			if  not set(job['tasks']).issubset(set(res.keys())):
				if  not set(job['tasks']).issubset(set(res.keys()).union(['NaN'])):
                			raise Exception("Result task names %s did not match job task names %s." % (res.keys(), job['tasks']))
...
def python_launcher(job):
    ...
    # If job['params'] is a list, we are in the case of Parallel BO, else, everything is normal.
    if isinstance(job['params'], list):
	params = [paramify_no_types(suggested_params_of_point) for suggested_params_of_point in job['params']]
    else:
    ...
    # If job['params'] is a list, we are in the case of Parallel BO, else, everything is normal.
    if isinstance(job['params'], list):
	result = [module.main(job['id'], suggested_params_of_point) for suggested_params_of_point in params]
    else: 
