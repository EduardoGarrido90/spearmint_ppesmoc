...
class Matern52(AbstractKernel):
    def __init__(self, num_dims, value=None, name='Matern52', prior=None, conditioning=None):
        self.name     = name
    ...
    def cross_cov(self, inputs_1, inputs_2):
        r2  = np.abs(kernel_utils.dist2(self.ls.value, inputs_1, inputs_2, self.conditioning))
        r   = np.sqrt(r2)
        cov = (1.0 + SQRT_5*r + (5.0/3.0)*r2) * np.exp(-SQRT_5*r)
	#if inputs_1.shape[0] == 8 and not kernel_utils.check_positive_definite_matrix(cov):
	#	import pdb; pdb.set_trace();
	#return kernel_utils.ensure_PSD_covariance_matrix(cov, self.conditioning)
	return cov
    ...
    def cross_cov_grad_data(self, inputs_1, inputs_2):
        ...
        return grad_r2[:,:,np.newaxis] * kernel_utils.grad_dist2(self.ls.value, inputs_1, inputs_2, self.conditioning)
