...
class Integer(AbstractTransformation):
    ...
    def forward_pass(self, inputs):
        ...
        # This is a faster version of below

        for j in range(inputs_to_convert.shape[ 1 ]):
            values = self.intervals[ j ][ np.argmin(cdist(np.array([inputs_to_convert[ :, j ]]).T, np.array([self.intervals[ j ]]).T), 1) ]
            inputs_to_convert[ :, j ] = values
            inputs[ : , self.integer_dimensions ] = inputs_to_convert
