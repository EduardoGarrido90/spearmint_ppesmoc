...
for a_value in a_grid:
        for b_value in b_grid:
        	for c_value in c_grid:
				gbfk = Br3(a_value,b_value,c_value)
                		wo = fixed_objective_function(gbfk.cdf(final_space))
...
#Plotting.
ax = plt.subplot(111)
gbfk = Br3(a_dewarp,b_dewarp,c_dewarp)
wo = fixed_objective_function(gbfk.cdf(final_space))
beta_warp = beta.cdf(input_space, a_warped, b_warped)
beta_dewarp = gbfk.cdf(input_space)
