...
def parse_config_file(config_file_dir, config_file_name, verbose=True):
    ...
    for task_name, task_opts in options['tasks'].iteritems():
        if "transformations" not in task_opts:
            ...
            if options["acquisition"] == "PES" or options["acquisition"] == "PESM" or options["acquisition"] == "PESMC" \
						or options["acquisition"] == "PPESMOC":
    ...
    if numObjectives > 1 and not (options["acquisition"] in set(['PESM','ParEGO', 'RANDOM', 'EHI', 'SUR', 'SMSego', 'PESMC', 'BMOO', 'PPESMOC', 'parallel_RANDOM', 'parallel_sequential'])):
...
def parse_tasks_from_jobs(jobs, experiment_name, options, input_space):
    ...
    if jobs:
        ...
        for task_name, task in tasks.iteritems():
			...
			vectorified_params = input_space.vectorify(job['params'])
			if isinstance(vectorified_params, list):
                        	task.inputs = np.append(task.inputs, np.array(vectorified_params),axis=0)
			else:
                        	task.inputs = np.append(task.inputs, vectorified_params[None],axis=0)
			if isinstance(job['values'], list):
				for value in job['values']:
                        		task.values = np.append(task.values, value[task_name])
			else:
