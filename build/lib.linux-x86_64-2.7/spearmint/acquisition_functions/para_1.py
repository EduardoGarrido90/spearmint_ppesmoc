...
# n_obs, n_pset, n_total: Counters of the points.
############################################################################################
def build_set_of_points_that_conditions_GPs(obj_models, con_models, all_tasks, all_constraints, pareto_set):
    X = np.array([np.array([])])
    for t in all_tasks:
        Xtask = obj_models[ t ].observed_inputs
	X = np.array([Xtask[ 0, ]])
...
def assign_jitter(obj_models, con_models, all_tasks, all_constraints):
    jitter = dict()
...
# a: Data structure. 
############################################################################################
def create_data_structure_for_EP_computations_and_posterior_approximation(obj_models, con_models, n_obs, n_pset, n_total, q, c, \
	...
	#Objective and constraint conditional predictive distributions.
        ...
        'Vinv'     : defaultdict(lambda: np.zeros((n_total, n_total))), #Inverse of the Covariance matrix of the conditional predictive distribution.
        ...
        'V_cons'        : defaultdict(lambda: np.zeros((n_total, n_total))), #Covariance matrix of the conditional predictive distribution.
	...
	#Other data.
        ...
        'X'        : X #Set of points that the GP predict and over which the GP is conditioned the feasibility and dominance factors approxed. by EP.
    }
    return a
...
def ep(obj_models, pareto_set, minimize=True, con_models=[], input_space=None):
    ...
    X, n_obs, n_pset, n_total = build_set_of_points_that_conditions_GPs(obj_models, con_models, all_tasks, all_constraints, pareto_set)
    q = len(all_tasks)
    ...
    jitter = assign_jitter(obj_models, con_models, all_tasks, all_constraints)
        
    # We create the posterior approximation
    a = create_data_structure_for_EP_computations_and_posterior_approximation(obj_models, con_models, n_obs, n_pset, n_total, q, c, \
                        mPred, Vpred, VpredInv, cholKstarstar, mPred_cons, Vpred_cons, VpredInv_cons, cholKstarstar_cons, jitter, X)
    
    # We condition the PD to the PESMOC conditions.
    a = perform_EP_algorithm(a, minimize, all_tasks, all_constraints)
    a = compute_choleskys(a, all_tasks, all_constraints)
    return a
...
def predictEP_multiple_iter_optim_robust(obj_models, con_models, a, pareto_set, Xtest, damping = 1, n_iters = 5, no_negatives = True, minimize=True):
    ...
    #o means old. f means objectives and c means constraints...
...
class PPESMOC(AbstractAcquisitionFunction):
	...
	# of the factors that do not depend on the candidate and other useful information.
	#################################################################################################
	def compute_ep_solution_for_not_dependant_on_candidate_factors(self, pareto_set, obj_model_dict, minimize, con_models_dict, key):
	...
	def acquisition(self, obj_model_dict, con_models_dict, cand, current_best, compute_grad, minimize=True, tasks=None, tasks_values=None):
		...
		epSolution = self.compute_ep_solution_for_not_dependant_on_candidate_factors(pareto_set, obj_model_dict, \
											minimize, con_models_dict, key)
		#We evaluate the acquisition function.
		acq_dict = self.evaluate_acquisition_function_and_compute_EP_factors_dependant_on_candidate(key, obj_model_dict, \
									cand, epSolution, pareto_set, minimize, con_models_dict)
	...
	######################################################################################################################
	def evaluate_acquisition_function_given_EP_solution(self, obj_models_dict, cand, epSolution, pareto_set, minimize=True, opt = None, con_models = {}):

	    if cand.ndim == 1:
        	cand = cand[None]
	    acq, unconstrainedVariances, constrainedVariances = self.compute_unconstrained_variances_and_init_acq_fun(obj_models_dict, cand, con_models)
