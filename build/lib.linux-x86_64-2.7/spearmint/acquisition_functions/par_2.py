...
class PPESMOC_2(AbstractAcquisitionFunction):
		...
		# We approximate the factors that do not depend on the candidate by using EP.
		    ...
		    #Consider to do this if bugs are unbeatable.
	            ...
	            acq_grad = self.compute_gradients_wrt_test_points(obj_model_dict, con_model_dict, pareto_set, tasks, cand, list_a)
                    ...
                    #METODO DE TEST DE DANIEL, DESCOMENTAR PARA PROBAR LOS GRADIENTES.
...
#            import pdb; pdb.set_trace()
	...
	def ppesmoc_computation_for_pareto_set_sample(self, obj_model_dict, pareto_set, minimize, con_models_dict, input_space, cand, \
            ...
            if key in self.reuse_EP_solution:
                ...
                a['ghfhat'] = a_old['ghfhat'].copy()   
                a['hhfhat'] =  a_old['hhfhat'].copy()   
                a['g_c_hfhat'] = a_old['g_c_hfhat'].copy()   
                a['h_c_hfhat'] = a_old['h_c_hfhat'].copy()   
	...
	def compute_full_updates_and_reduce_damping_if_fail(self, a, damping, minimize, aOld):
		update_correct = False
	        ...
	        while update_correct == False:
	            ...
	            try:
	                aNew = update_full_Factors(copy.deepcopy(a), damping_inner, minimize = minimize, no_negative_variances_nor_nands = False)
