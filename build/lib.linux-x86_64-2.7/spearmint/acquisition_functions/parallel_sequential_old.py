...
class parallel_sequential(AbstractAcquisitionFunction):
	...
	def __init__(self, num_dims, verbose=True, input_space=None, grid=None, opt = None):
        ...
        #Gets the solution for parallel sequential when there is no need to optimize the acq. fun.
        def get_cached_computed_points(self, key):
            return self.cached_computed_points[key]
                ...
                if not key in self.cached_computed_points:
                    ...
                    for i in range(self.options["parallel_sequential_evaluations"]):
                        ...
                        if i == 0: #Nothing to pend, necessary to generate grid for suggest.
                            self.chooser.set_black_boxes(tasks_values)
                            self.chooser.generate_grid(tasks_values)
